...
> 我演示的是一个旅行社智能顾问 Agent（智能体），不是普通 RAG（检索增强生成）问答。主 Agent 通过阶段状态、目的地 Router（路由器）和嵌套交通 Coordinator（协调器）编排能力，用 MCP（模型上下文协议）接外部工具，最后输出结构化 `report_data`（结构化报告数据）。当前只有审批治理骨架，还没有 LangGraph `interrupt/resume`（中断/恢复）闭环。
...
5. 展示圆周旅迹级旅程工作台：
   - `planning_trace`（规划过程轨迹）：公开攻略搜索、地点收集、天气核验、路线计算和每日编排。
   - `journey_plan.v1`：分日路线、地图、日期 Tab、POI（兴趣点/地点）hover 名称、路线说明和待核验项；普通用户视图不再弹出旧版底部 POI 详情卡。
   - 地图高德优先，缺少 `AMAP_WEB_JS_KEY`（高德浏览器公开 Key，密钥/标识符）时降级到 Leaflet（交互地图前端库）/ OSM（OpenStreetMap，开放街图）。
...
| 追问 | 30 秒回答 |
|---|---|
| Agent 和 workflow（工作流）怎么取舍？ | 旅行规划有明确阶段，所以用 `TravelState`、中间件和迁移工具控制逻辑阶段；每个阶段内部保留 Agent 推理和工具选择弹性。主业务不是手写八节点 `StateGraph`，目的地 Router 才是显式状态图。 |
...
| HITL 做完了吗？ | 没有。当前有审批策略、持久化记录和事件审计，但没有 `interrupt/resume`、审批结果回写 checkpoint（检查点）和原工具调用恢复。 |
