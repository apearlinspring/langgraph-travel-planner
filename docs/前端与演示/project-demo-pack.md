...
> 这个项目把用户的一次旅行咨询拆成阶段化状态机，由主控 Agent 编排目的地 Router（路由器）、交通 Coordinator（协调器）及查询工具、RAG 知识、MCP（模型上下文协议）工具、HITL（人类在环）治理骨架和最终 `report_data`（结构化报告数据）交付，因此重点不是“回答得像不像”，而是“能不能按顾问流程稳定交付、可审计、可复跑”。
...
|---|---|---|---|
| Agent 能力编排 | `app/agents/handoffs/travel_agent.py`、`app/agents/routers/destination_router.py`、`app/agents/subagents/transport_coordinator.py` | 主控 Agent 负责旅行流程，目的地 Router 负责攻略和天气分流，交通 Coordinator（协调器）直接调用航班、高铁、自驾查询工具。主流程不是“每个阶段一个 Agent”，也没有三个交通方式子 Agent 的运行链路。 | `.\.venv\Scripts\python -m pytest tests\test_travel_agent_tool_registry.py -q` |
...
| HITL 治理骨架 | `app/core/approval.py`、`app/core/permissions.py`、`app/api/v1/approvals.py`、`docs/治理与可观测/approval-governance.md` | 当前不做真实支付或预订，已有审批策略、事件账本和 readiness（就绪状态）语义；尚未接成 LangGraph `interrupt/resume`（中断/恢复）闭环。 | `.\.venv\Scripts\python -m pytest tests\test_approval_governance.py -q` |
| 可观测性 | `app/core/observability.py`、`app/evaluation/runtime_metrics.py`、`docs/治理与可观测/observability.md` | 每轮对话输出 `first_token_seconds`、`total_elapsed_seconds`、工具调用数、失败数、fallback（兜底）数和 token 估算；首个助手片段可能只是固定 ACK（确认收到），不等于 LLM（大语言模型）有意义内容或完整处理完成，演示时必须同时展示总耗时。 | `.\.venv\Scripts\python -m pytest tests\test_runtime_metrics.py -q` |
| 验收门禁 | `app/evaluation/acceptance_gate.py`、`scripts/run_evaluation_scenarios.py`、`docs/评估与验收/evaluation-system.md` | 确定性场景门禁检查 `report_data`、RAG 证据、工具治理、失败/兜底预算、运行预算和旅行社业务证据；它不能单独证明轨迹最优、线上长期稳定或用户体验优秀。 | `.\.venv\Scripts\python scripts\run_evaluation_scenarios.py --acceptance-smoke --dry-run` |
...
| 前端报告 | `frontend/app.js`、`frontend/zhixing.html`、`docs/前端与演示/frontend-report-experience.md`、`docs/前端与演示/report-data-delivery-contract.md` | 前端优先消费结构化 `report_data`，展示预算明细、待核验项、地图路线和导出边界；导出 HTML 不代表支付、预订、锁价或履约。 | `node scripts\verify_frontend_report_renderer.js`；`node scripts\verify_frontend_browser_regression.js` |
| 核心验收证据 | `docs/评估与验收/acceptance-core-report.md`、`docs/评估与验收/live-acceptance-runbook.md`、`app/evaluation/acceptance_gate.py` | 2026-07-12 的 `final-core-6` 在同一后端代码快照和同一组真实本地依赖上完整执行 9 个场景，9/9 同时通过报告质量、Agent 工业指标和运行预算门禁；它仍只是未提交工作树的一次本地统一跑批，不等于干净 commit 可复现、重复运行稳定或生产就绪。 | `.\.venv\Scripts\python scripts\run_evaluation_scenarios.py --acceptance-core --preflight-only --json --no-summary` |
...
- `docs/评估与验收/acceptance-core-report.md` 同时保留 2026-07-12 当前单次统一跑批与 2026-05-17 旧门禁历史记录。演示时可以引用前者，但必须同时说明工作树未提交、尚未绑定干净 commit、未验证重复运行分布，也不代表目标生产环境就绪；对外发布前应冻结 commit 后重新执行 smoke 和完整 core。
...
- `docs/前端与演示/report-data-delivery-contract.md` 记录 `report_data` 到前端报告、复制摘要和导出 HTML 的交付契约；它说明导出件是结构化报告快照，不是交易凭证、订单凭证或履约凭证。
...
适用场景：审阅者追问“如何证明不是只跑 smoke（烟测）”时，展示 acceptance-core 的可审计入口、历史状态地图和当前重跑要求。
...
- `docs/评估与验收/acceptance-core-report.md` 顶部记录 2026-07-12 当前未提交工作树的 `final-core-6` 统一 9/9 状态地图，后文保留 2026-05-17 旧门禁历史证据；前者只证明一次本地完整跑批，不代表干净 commit 可复现或生产就绪。
...
2. 编排方式：主控 Agent + 目的地 StateGraph Router（显式状态图路由器）+ 交通 Coordinator + 状态迁移工具。
3. 证据来源：公开 RAG、内部 RAG、产品化路线样板、MCP 真实查询、用户长期记忆和规则估算。
4. 交付物：最终不是散文答案，而是结构化 `report_data` 和可导出的报告。
5. 治理边界：真实库存、锁价、支付、短信和客服不在当前能力内；当前审批记录不会自动中断或恢复 Agent。未来接入敏感动作前必须补齐 HITL、幂等和补偿。
6. 验收方式：用确定性场景门禁判断报告、证据、工具失败/兜底预算、运行预算和前端导出准备度，并用人工 badcase（坏案例）复核补足轨迹质量判断。
