...
- 地图预览 API（应用程序接口）现在会接收分日 `segments` 中的路段偏好，并把 `selected_mode`、`recommended_mode`、`locked_by_user`、`verification_status` 和 `verification_label` 回填到前端编辑台；当前只有打车/驾车偏好复用现有高德驾车路线核验，公交/地铁、步行、铁路和航班仍标记为“待高德路线核验”，不冒用驾车距离或用时。
