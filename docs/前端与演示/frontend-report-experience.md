...
- 人工确认边界：`tool_audit_summary.approval` 或 `evidence_bundle.approval_governance` 会渲染为“人工确认与不可承诺项”，明确订单号、模拟确认跳转或报告导出不代表真实支付、真实预订、真实锁价或履约成功。
...
- 人工确认边界：说明当前只支持站内模拟确认跳转，不会自动支付、发短信或真实下单。
...
- `frontend/report-data-itinerary.js`：结构化报告每日行程、路线点、路线段和未就绪空状态渲染。
- `frontend/report-text-structured.js`：文本报告结构化卡片、旅程预览和行程摘要组合渲染。
...
- 模块化目标是让 `frontend/app.js` 最终只保留全局状态、页面初始化、基础工具函数和模块接线；这不是当前完成状态。主文件仍接近 7000 行，并保留大量地图、治理、认证、会话和渲染遗留逻辑，应继续视为前端巨石债务。
- 新增前台能力时，优先判断它属于请求层、交互层还是渲染层，再放进现有模块；不要把新逻辑直接塞回 `app.js`。
- 后续只做有明确边界和回归覆盖的局部迁移；如果某个模块继续膨胀到接近 1000 行，优先拆出高内聚职责，而不是恢复“单文件全包”或为了行数机械切碎。
...
- “演示记录”入口和 M1 模拟确认页只创建或展示站内演示状态，不接真实支付网关，不扣款、不锁库存、不生成第三方支付链接。
