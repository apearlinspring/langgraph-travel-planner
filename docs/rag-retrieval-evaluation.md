...
这份评估补的是 RAG（检索增强生成）链路里的“能不能把问题召回到正确知识源”证据。它不调用真实 LLM（大语言模型）、不连接向量库、不读取 `.env`，而是基于仓库内的公开目的地知识和旅行社内部知识，构造一个可复跑的小型标注集，用于回答项目审阅里常见的“RAG 怎么验证召回效果”。

评估不是生产级全量 benchmark（基准测试），也不替代真实 embedding（向量嵌入）召回压测；它的价值是给当前项目提供一组清楚、可复跑、可解释的 Top-K 召回指标。

## 标注集

场景文件位于 `data/evaluation/rag_retrieval_scenarios.json`，当前覆盖 12 个查询：

- 公开目的地攻略：西安历史文化、美食、自由行攻略。
- 旅行社产品：亲子省心、银发低强度、团建透明预算、情侣氛围路线、自由规划边界和成熟路线结构。
...
- `baseline_bm25`：只用正文做 BM25（词频检索算法）排序，作为纯正文关键词基线。
- `metadata_aware_bm25`：正文加 metadata（元数据）字段，并按查询中的旅行社、报价、风险、报告、自由行、产品匹配字段等语义提示做轻量分类加权，模拟项目线上 RAG 的“正文 + 元数据契约”设计。
...
> 为旅行社内部知识 RAG 建立小型离线召回评估集，覆盖目的地攻略、产品路线、SOP、报价、风险和报告交付 6 类知识，使用 Top-K source recall、category recall 和 MRR 对比纯正文 BM25 与元数据增强检索策略，用可复跑指标验证知识库组织与检索策略改进。

如果要写百分比，必须以最新脚本输出为准。建议表述为“在 12 条标注查询的小型评估集上，metadata-aware 检索相对纯正文 BM25 提升了 source recall@3 / category recall@3 / MRR”，不要泛化成大规模线上效果或真实用户指标。

## 当前复跑结果

2026-05-16 使用默认 12 条标注查询、16 份本地知识文档复跑结果：

| strategy | source recall@3 | category recall@3 | source type recall@3 | hit rate@3 | MRR |
|---|---:|---:|---:|---:|---:|
| baseline_bm25 | 83.33% | 87.50% | 91.67% | 91.67% | 0.9167 |
| metadata_aware_bm25 | 95.83% | 100.00% | 100.00% | 100.00% | 0.9444 |

可用于简历的保守描述：

> 在 12 条小型标注查询上，元数据增强检索的 source recall@3 从 83.33% 提升到 95.83%，category recall@3 从 87.50% 提升到 100.00%，MRR 从 0.9167 提升到 0.9444。

## 当前边界

- 标注集规模小，主要用于项目自证和项目讲解，不代表全量线上查询分布。
