...
当前阶段已经增加旅行社租户、成员、客户关系、供应商产品、报价、订单、内部订单审核、订单事件、幂等记录、支付尝试和履约记录的数据模型与迁移，并加入角色和外部动作配置门禁。`/api/v1/agency` 的内部 API 支持报价草稿、发布、客户接受、订单草稿、提交审核，以及专职租户审批员批准或拒绝；创建报价要求客户在对应旅行社的关系为 `active`，但客户导入、同意和关系激活/停用 API 尚未实现。规划报告不会自动转成报价或自动通过审核；内部 `approved` 也不触发供应商、支付、退款或通知动作。真实外部执行尚未接入，相关总开关和细粒度开关默认关闭。
