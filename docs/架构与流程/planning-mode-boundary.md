...
当前阶段已经增加旅行社租户、成员、门店、门店岗位授权、客户关系、安全认领、只追加同意记录、主顾问分配、供应商产品、报价、订单、内部订单审核、人工取消案件、补偿结果、独立对账、事件、幂等、支付尝试和履约记录的数据模型与迁移，并加入应用层门店范围和外部动作配置门禁。`/api/v1/agency` 已支持目标账户认领、服务端同意、报价/订单审核，以及平台外人工取消结果的分岗登记与独立核验；旅行社 API 只有在数据库提交和延迟约束通过后才发送成功响应。取消案件的 `completed` 可能是无外部暴露订单经审批后直接完成，也可能是最新所需人工结果已在平台内独立匹配；所有外部动作标记始终为关闭。批量导入、邀请投递、客户通知、真实身份核验、法律合规闭环、PII（个人可识别信息）档案、跨门店转移和门店停用/关闭 API 仍未实现。规划报告不会自动转成报价或通过审核；内部 `approved`、`cancelled` 或取消 `completed` 都不触发供应商、支付、退款或通知动作。
