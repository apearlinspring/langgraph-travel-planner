...
1. 在隔离 PostgreSQL 验证 `0001 -> 0002 -> 0003 -> 0004 -> 0005 -> 0006` 全链迁移、非空旧数据 provenance 回填、升级/降级、token/同意记录不可变、跨租户与跨门店越权、并发认领、事务回滚；应用层门店授权仍需评估数据库 RLS 或独立策略引擎。
...
9. 在目标环境绑定 commit，真实执行 `0001 -> 0002 -> 0003 -> 0004 -> 0005 -> 0006` 迁移、备份恢复、并发、回调重放、故障恢复和小流量验收，再评估是否放行单个真实动作。
