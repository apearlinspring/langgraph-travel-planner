...
- 行程与地图 20 分：检查每日行程数量、每日内容、路线节点、`map_routes` 和可选 `route_map.days` 是否可用于前端展示。
- 预算解释 20 分：检查总预算、交通/住宿/餐饮/景点体验/服务预留/其他六类预算、费用依据、预算置信度和待核验项；展示层不默认要求或输出人均预算。
- 风险与调整 15 分：检查天气、交通、酒店、预约、Plan B 和后续调整建议。
- 旅行社业务贴合 15 分：检查内部知识库来源、自由规划 / 省心方案模式、业务亮点和知识分类。
- 前端导出准备 10 分：检查地图标签、每日路线、分日路线卡和导出章节是否能支撑 HTML/PDF/图片导出。
...
- 第一版不判断景点路线是否地理最优，只检查路线节点、每日地图数据和 `route_map.days` 分日路线点是否存在并可展示。
