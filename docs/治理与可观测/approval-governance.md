...
- 独立的旅行社内部订单审核：订单送审前必须存在一名排除业务发起人和订单客户的有效专职 `approver`；开放取消案件会阻止送审，待处理业务撤权时必须逐笔保留合格替代审批员。该角色可处理本门店 `pending_review`，且不能自审。批准还要求客户保持 `active + granted`，客户停用后保留中的审核只能拒绝，且拒绝前客户关系不能重新激活。这不是平台 Approval/HITL。
- 独立的人工取消治理：案件创建后即冻结原支付/履约账本，专职 `approver` 审批，`booking_operator`/`finance` 分岗登记平台外结果，不同 `auditor` 从专用脱敏队列发现记录并独立提交退款观察金额/币种和证据后对账；所有外部动作标记保持关闭。这同样不是平台 Approval/HITL，也不会自动执行退款或供应商取消。
...
旅行社业务域已增加 `agency`、`agency_membership`、`agency_branch`、`agency_branch_role_grant`、`agency_customer`、`agency_customer_invitation`、`agency_customer_consent_record`、`agency_customer_event`、`agency_customer_advisor_assignment`、`agency_quote`、`agency_order`、`agency_order_review`、取消案件/事件、人工补偿结果、独立对账、`agency_order_event`、`idempotency_record`、`payment_attempt` 和 `fulfillment_record` 等持久化模型。`agency_order_review` 已实现旅行社内部审核闭环；取消域另以强绑定、只追加记录和职责分离保存平台外人工结果。
...
取消路由同样挂载在 `/api/v1/agency`，但与订单审核、平台 Approval 三者相互独立。客户本人、当前主顾问、同门店经理或全域管理员可以发起，但建案时必须已有排除案件发起人和订单客户的 eligible approver；同门店专职 `approver` 审批且不能是发起人或订单客户；`booking_operator` 只登记人工取得的供应商取消结果，`finance` 只登记财务结果，另一名 `auditor` 核验且不能自我对账。`owner/admin` 不能替代专职审批员、执行岗位或审计岗位。

服务端从锁定的订单、支付尝试和履约记录派生 `supplier_cancel_required` / `refund_required`，客户端不能扩大或缩小范围。开放取消案会阻止原订单进入 `pending_review`；撤销审批授权时，待处理订单审核与 `approval_pending` 取消案按每笔业务分别检查，撤权后仍须存在排除该业务发起人和订单客户的合格替代审批员。数据库在取消审批决定时还会复验旅行社、门店、`agency_membership` 和门店授权均为 `active`，且成员与授权角色均严格为 `approver`；`0007` 同时冻结 `agency_membership.id/agency_id/user_id/created_at` 身份绑定，防止改绑账户或旅行社。`refund_required` 表示财务现状和证据必须登记核验，不是退款执行命令。外部引用与证据只以 SHA-256 摘要进入只追加记录；统一响应不返回摘要、原因明细、内部审批备注或事件元数据。两项 `required` 均为 `false` 时，专职审批员可直接把符合条件的 `draft`/`approved` 订单在平台内取消并完成案件；存在必需动作时，只有最新所需记录均由不同审计人员核验为 `matched` 才能完成。两条路径都不会调用供应商、支付或通知适配器。
...
| 旅行社租户 | `booking_operator` | 只可登记平台外人工供应商取消结果；当前没有供应商调用入口。 |
| 旅行社租户 | `approver` | 当前唯一可读取自己有效授权门店的审核工作队列并决定内部订单审核或取消申请；必须属于同一有效旅行社和有效门店，成员与门店授权均有效，且不能审核本人订单或本人发起的业务。不能据此调用平台审批全量接口。 |
| 旅行社租户 | `finance`、`auditor` | `finance` 只登记平台外财务结果，`auditor` 独立核验且不得自我对账；两者都不能决定订单审核或执行真实支付/退款。 |
| 旅行社租户 | `admin`、`owner` | 旅行社全域管理角色，可管理门店、门店授权和客户；高风险动作仍需职责分离，不能代替专职审批员。 |

旅行社门店/客户授权由应用服务的对象检查和 SQL 可见性过滤器实现，不是 PostgreSQL RLS。授权敏感写入会对门店和成员范围持有共享行锁，使并发撤销岗位授权或改变门店状态必须等待，避免 TOCTOU（检查与使用时序差）竞态。`0007` 只为成员身份改绑、合格审批员可用性、开放取消案送审冲突及取消审批人资格增加了定向 PostgreSQL 触发器门禁，不等于所有行级可见性都已由数据库强制。生产数据库账号一旦绕过服务层，其他过滤器和锁顺序不会自动生效；因此还需要最小权限数据库账号、系统化越权测试，并评估数据库 RLS 或独立策略引擎。
...
- 人工取消接口只接收 64 位十六进制外部引用/证据摘要，不接收或返回原始供应商编号、退款凭证、原因明细和内部审核备注；摘要是审计关联原语，不证明外部动作真实、合法或不可抵赖。
...
- 不提供批量客户导入、邀请投递/客户通知、真实身份核验、客户 PII 档案与法律级同意流程，也不提供双门店经理审批或自动化关店编排。

安全客户认领、服务端同意记录、门店、客户生命周期、顾问分配、客户停用时的内部交易收口、内部报价/订单审核、旅行社管理员执行的客户原子转店、门店 `active → inactive → closed` 清场治理，以及 `cancellation_pending` / `manual_intervention` 的平台外人工结果登记和独立对账已经存在。转店保留历史业务原门店归属，`inactive` 门店只能处理存量清场，`closed` 不可逆；这些内部治理动作不会通知客户或调用外部系统。真实业务接入前仍需补齐邀请投递与通知、真实身份核验、PII/同意合规、双门店经理审批、平台审批绑定、外部适配器、回调验签、跨系统补偿和自动对账。真实供应商预订、取消、支付、退款和通知继续 fail-closed（默认拒绝）。
