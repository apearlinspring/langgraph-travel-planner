...
安全客户认领、服务端同意记录、门店、客户生命周期、顾问分配、客户停用时的内部交易收口、内部报价/订单审核，以及 `cancellation_pending` / `manual_intervention` 的平台外人工结果登记和独立对账已经存在；真实业务接入前仍需补齐邀请投递与通知、真实身份核验、PII/同意合规、转店与门店状态管理、平台审批绑定、外部适配器、回调验签、跨系统补偿和自动对账。真实供应商预订、取消、支付、退款和通知继续 fail-closed（默认拒绝）。
