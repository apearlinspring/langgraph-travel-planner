...
- 需求收集首轮遇到复杂慢请求时优先轻量响应：如果用户第一句话已经同时包含酒店、天气、交通、风险、老人低压力、兜底或完整旅行社省心方案，StepConfigMiddleware 会暂缓所有工具，只确认理解并说明后续会核验真实证据。用户确认后再调用 `record_requirement_tool` 和对应真实工具，以控制首轮完整处理耗时和外部工具压力；固定 ACK（确认收到）只提供连接首响，不能用来证明有意义内容已经生成。
- 首轮轻量响应会把用户原始诉求和规划模式线索暂存到状态中，后续 `record_requirement_tool` 会纳入这些线索推断 `agency_plan` 或 `free_planning`，避免因首轮不调用工具而丢失模式边界。
- 首次进入完整 `agency_plan` 且出发地、目的地、日期、天数、人数和预算等基础事实已经齐全时，API 可以先发送固定 ACK（确认收到）片段“收到，正在为你整理完整省心方案。”，随后仍在同一轮创建并运行完整 Travel Agent。该 ACK 只表示请求已接收，不是省心方案正文、LLM（大语言模型）有效推理开始或报告完成；后续 Agent、工具审计、状态推进和 `report_data` 契约都不能省略。
...
- 交通方案刚写入后，即使工作流已经推进到住宿阶段，中间件也会在同一轮暂时移除 `query_hotel_options` 和住宿选择工具，避免一轮内连续执行“查交通 -> 选交通 -> 查酒店 -> 选酒店”的长链路挤占单轮总耗时和工具调用预算。验收脚本下一条住宿确认消息会重新开放酒店查询。
...
以下是当时版本和旧门禁下的完整 acceptance-core（核心验收）单次跑批记录，不代表当前版本仍通过：
...
该历史轮次没有修改 runtime budget（运行预算）阈值或 warning ratio（警戒比例）。当前门禁已经增加工具失败数、失败率和 fallback 上限；必须按当前代码重跑后才能给出新结论。`.runtime/` 原始快照仅本地保留，不提交。
