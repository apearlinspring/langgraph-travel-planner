...
| RAG 怎么保证旅行社业务感？ | 内部知识库按产品、SOP（标准作业流程）、报价、风险和报告标准组织，最终报告会保留 evidence bundle（证据包）和业务证据。 | `data/documents/internal/`、`app/tools/rag_tools.py`、`app/evaluation/rag_quality.py` | `.\.venv\Scripts\python scripts\validate_rag_knowledge.py` | 内部知识不能暴露给用户为“内部资料原文”。 |
