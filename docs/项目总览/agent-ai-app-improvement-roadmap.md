...
| 旅行社客户与门店权限 | 门店、角色授权、安全认领、服务端同意、客户生命周期、主顾问分配、客户转店和门店停业治理已实现候选。`0008` 允许 owner/admin 立即原子转移客户当前门店；`active`、`inactive`、`blocked` 客户都可转移，历史报价、订单、取消等记录保留原门店，`active` 客户可同时指定目标门店主顾问。门店 `active -> inactive -> closed`：`inactive` 是停止新业务、继续清场的停业期；`closed` 不可逆，且要求当前客户（包括 `inactive`/`blocked`）、待处理邀请、有效顾问分配/岗位授权、待审核订单、开放报价/订单/取消案件全部归零。当前仍是应用层行级授权，不是 PostgreSQL RLS，也没有跨门店经理双边审批、邀请投递/客户通知、真实身份核验、PII 档案或法律级同意 | 对 `0008` 做 PostgreSQL 17 CI、目标环境迁移/恢复和并发锁复验；后续再设计源门店与目标门店经理双边审批。转店和关店只改变平台内当前归属/治理状态，不发送通知，也不改写历史门店记录或触发外部订单动作 | P0 | `tests/test_agency_branch_transfer_closure_api.py`、`tests/test_agency_branch_transfer_closure_service.py`、`tests/test_agency_branch_transfer_closure_migration.py`、`tests/test_branch_drain_authorization.py`；`0008` PostgreSQL CI 与目标环境证据待补，不能沿用 `0007` 通过数字 |
| 旅行社人工取消与对账 | 旧 `cancellation_pending` / `manual_intervention` 已有状态但无退出入口；现新增取消案件、服务端暴露派生、专职审批、预订/财务分岗登记、不同审计人员核验、最新结果重做、只追加事件和数据库一致性门禁。当前只记录平台外人工结果，不执行真实供应商取消或退款 | `0007` 已在默认与 PostgreSQL 17 CI 双绿；`0008` 转店/关店实现候选进入数据库与目标环境验证阶段。后续真实适配器仍必须绑定平台 Approval/HITL、回调验签、幂等、补偿和自动对账 | P0 | [`e17b97d`](https://github.com/apearlinspring/langgraph-travel-planner/commit/e17b97d82c24b7f5271973cc8f18e884124b7d6b) / [Actions 30602058425](https://github.com/apearlinspring/langgraph-travel-planner/actions/runs/30602058425)：默认 `1841 passed, 49 deselected`，PostgreSQL 17 `25 passed`，只覆盖到 `0007`；目标环境仍待验证 |
...
- 实现候选 `20ff71592096dfb4fc718cef050832a745bfe174` / 运行 `30534862434` 是 `0004` 历史基线；首次包含 `0005` 的运行 `30542366036` 暴露触发器字段访问缺陷，不能写成通过；`0006` 修正提交 `b8b8bea29477b472c942b7df40e8da6e9dbf05ab` / 运行 `30551146157` 已验证默认 `1738 passed, 39 deselected`、PostgreSQL 17 四文件 `15 passed`。`0007` 实现提交 `e17b97d82c24b7f5271973cc8f18e884124b7d6b` / 运行 `30602058425` 已验证默认 `1841 passed, 49 deselected`、PostgreSQL 17 五文件 `25 passed`。`0008` 是当前实现候选，尚无可引用的 PostgreSQL CI 通过结果，目标环境迁移、恢复和并发锁证据也未完成；任何历史 CI 都不能外推为 `0008` 或目标环境通过。
