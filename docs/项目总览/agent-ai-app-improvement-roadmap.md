...
- 实现候选 `20ff71592096dfb4fc718cef050832a745bfe174` 曾在运行 `30534862434` 通过 3 个文件共 10 项 PostgreSQL 17 测试；这是 `0004` 历史基线。首次包含 `0005` 的运行 `30542366036` 暴露共享触发器字段访问缺陷，因此不能写成通过；`0006` 修正提交 `b8b8bea29477b472c942b7df40e8da6e9dbf05ab` 已由运行 `30551146157` 验证为默认 `1738 passed, 39 deselected`、PostgreSQL 17 四文件 `15 passed`，但 CI 仍不能替代目标环境迁移。
