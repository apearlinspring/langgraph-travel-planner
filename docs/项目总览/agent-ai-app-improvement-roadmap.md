...
| 旅行社客户与门店权限 | 门店、角色授权、安全认领、服务端同意、客户生命周期、主顾问分配、客户转店和门店停业治理已实现，并取得一次性 PostgreSQL 17 CI 证据。`0008` 允许 owner/admin 立即原子转移客户当前门店；`active`、`inactive`、`blocked` 客户都可转移，历史报价、订单、取消等记录保留原门店，`active` 客户可同时指定目标门店主顾问。门店 `active -> inactive -> closed`：`inactive` 是停止新业务、继续清场的停业期；`closed` 不可逆，且要求当前客户（包括 `inactive`/`blocked`）、待处理邀请、有效顾问分配/岗位授权、待审核订单、开放报价/订单/取消案件全部归零。当前仍是应用层行级授权，不是 PostgreSQL RLS，也没有跨门店经理双边审批、邀请投递/客户通知、真实身份核验、PII 档案或法律级同意 | 对 `0008` 做目标环境迁移/恢复、复杂存量数据和并发锁复验；后续再设计源门店与目标门店经理双边审批。转店和关店只改变平台内当前归属/治理状态，不发送通知，也不改写历史门店记录或触发外部订单动作 | P0 | [`c574649`](https://github.com/apearlinspring/langgraph-travel-planner/commit/c5746496203f628fe9a93a91ebb998c910c2a920) / [Actions 30606856484](https://github.com/apearlinspring/langgraph-travel-planner/actions/runs/30606856484)：默认 `1878 passed, 58 deselected`，PostgreSQL 17 六文件 `34 passed`；目标环境和并发锁证据待补 |
| 旅行社人工取消与对账 | 旧 `cancellation_pending` / `manual_intervention` 已有状态但无退出入口；现新增取消案件、服务端暴露派生、专职审批、预订/财务分岗登记、不同审计人员核验、最新结果重做、只追加事件和数据库一致性门禁。当前只记录平台外人工结果，不执行真实供应商取消或退款 | `0008` 已在默认与 PostgreSQL 17 六文件 CI 双绿，下一阶段是目标环境迁移/恢复和并发锁验证。后续真实适配器仍必须绑定平台 Approval/HITL、回调验签、幂等、补偿和自动对账 | P0 | [`c574649`](https://github.com/apearlinspring/langgraph-travel-planner/commit/c5746496203f628fe9a93a91ebb998c910c2a920) / [Actions 30606856484](https://github.com/apearlinspring/langgraph-travel-planner/actions/runs/30606856484)：默认 `1878 passed, 58 deselected`，PostgreSQL 17 `34 passed`；目标环境仍待验证 |
...
| M1 首部署 dry-run | `check_m1_first_deploy_dry_run.py` 已提供不连接服务器、不上传文件的预演入口，并会对脏工作树或缺目标输入 fail-closed；包含 `0008` 的当前候选尚缺绑定最终提交和真实目标输入的通过记录 | 冻结干净 commit，补齐目标输入后执行 dry-run，复核发布工具、Compose、公开边界和远端命令计划 | P0 | `scripts/check_m1_first_deploy_dry_run.py --json`、`docs/部署与运行/m1-first-deploy-dry-run.md` 和绑定候选的通过摘要 |
| M1 发布包 manifest | `build_release_artifact.py` 已能从干净 Git `HEAD` 生成 archive 和 manifest 并记录 commit、tree、tracked file count 与 sha256；包含 `0008` 的当前候选尚未生成与最终提交绑定的可复现候选发布包 | 冻结并评审干净 commit，生成不可变发布包，校验哈希并让部署记录引用同一 artifact（发布制品） | P0 | `scripts/build_release_artifact.py --execute --output-dir <release-output-dir> --json`、发布包 manifest 和哈希复核 |
...
| 2026-07-30 | 旅行社客户安全认领与同意证据 | 新增 256-bit 高熵、24 小时过期、可撤销、单次使用且只存 SHA-256 摘要的目标账户认领；同一旅行社同 target 待邀请唯一。认证客户端先读取固定告知再回传预期版本/摘要；服务端生成只追加规范化证据。legacy 认领会重置旧同意并先停用原 active 关系；新激活/报价/订单要求 `secure_claim + server_canonical`，提交失败不得先返回 `2xx` | `0005` 功能与 `0006` 触发器修正已由运行 `30551146157` 在默认和 PostgreSQL 17 job 双绿验证 | 不提供邀请投递、真实身份核验或法律合规证明；CI 不能替代目标环境迁移、真实身份或法律证据 |
| 2026-07-30 | 旅行社人工取消与独立对账 | 为订单增加取消申请、专职审批、平台外人工供应商/财务结果登记、不同审计人员核验和人工介入恢复；`0007` 强制只追加、四眼制、最新结果匹配、订单/案件一致和外部动作关闭。案件 `requested_at` 记录申请创建，订单 `cancellation_requested_at` 仅在批准后进入取消处理时写入，订单 `cancelled_at` 只记录真正完成内部取消 | `e17b97d` / Actions `30602058425` 已验证默认 `1841 passed, 49 deselected` 与 PostgreSQL 17 `25 passed`；目标环境迁移仍待验证 | `completed` 可能是无外部暴露订单经审批后直接完成，也可能表示最新所需人工结果已独立匹配；两者都不代表供应商取消、银行退款或通知已发生 |
| 2026-07-31 | 旅行社客户转店与门店清场/关闭 | owner/admin 可把客户当前归属即时原子转到同旅行社 active 目标门店；`active`、`inactive`、`blocked` 客户可转移，历史分支记录保持不变，`active` 客户可选目标主顾问。门店先从 `active` 进入 `inactive` 清场期，停止新业务但保留拒绝待审核订单和取消收口能力；只有当前客户（含 `inactive`/`blocked`）、待邀请、有效分配/授权、待审核、开放报价/订单/取消案全部归零后才能不可逆 `closed` | `0008` 实现、API、服务、迁移和授权专项测试已完成一次性 CI 验证：`c574649` / Actions `30606856484`，默认 `1878 passed, 58 deselected`，PostgreSQL 17 六文件 `34 passed` | 这是旅行社内部归属和门店治理，不发送客户通知、不改外部订单；跨门店经理双边审批以及目标环境迁移、恢复、并发锁证据仍缺失 |
...
- 实现候选 `20ff71592096dfb4fc718cef050832a745bfe174` / 运行 `30534862434` 是 `0004` 历史基线；首次包含 `0005` 的运行 `30542366036` 暴露触发器字段访问缺陷，不能写成通过；`0006` 修正提交 `b8b8bea29477b472c942b7df40e8da6e9dbf05ab` / 运行 `30551146157` 已验证默认 `1738 passed, 39 deselected`、PostgreSQL 17 四文件 `15 passed`。`0007` 实现提交 `e17b97d82c24b7f5271973cc8f18e884124b7d6b` / 运行 `30602058425` 已验证默认 `1841 passed, 49 deselected`、PostgreSQL 17 五文件 `25 passed`。`0008` 实现提交 `c5746496203f628fe9a93a91ebb998c910c2a920` / 运行 `30606856484` 已验证默认 `1878 passed, 58 deselected`、PostgreSQL 17 六文件 `34 passed`；目标环境迁移、恢复和并发锁证据仍未完成，不能由一次性 CI 外推为目标环境或生产通过。
