...
| 旅行社人工取消与对账 | 旧 `cancellation_pending` / `manual_intervention` 已有状态但无退出入口；现新增取消案件、服务端暴露派生、专职审批、预订/财务分岗登记、不同审计人员核验、最新结果重做、只追加事件和数据库一致性门禁。当前只记录平台外人工结果，不执行真实供应商取消或退款 | 先让 `0007` 在默认与 PostgreSQL 17 CI 双绿，再进入客户跨门店转移和门店 `closing/closed`；后续真实适配器必须绑定平台 Approval/HITL、回调验签、幂等、补偿和自动对账 | P0 | `tests/test_agency_cancellation_*.py`、`20260730_0007` 迁移；本地单元/静态证据已通过，推送 CI 与目标环境仍待验证 |
...
| 2026-07-30 | 人工取消、补偿结果与独立对账 | 新增 9 个取消域操作、4 张强绑定表和 `0007` 门禁；服务端派生所需证据，专职审批，`booking_operator`/`finance` 分岗登记，脱敏结果队列向不同 `auditor` 暴露待办记录 ID 与对账状态，失败/未知/不匹配进入人工介入，最新结果可重做。所有外部动作固定为关闭 | `app/agency/cancellation_service.py`、`cancellation_support.py`、取消 API/模型、`20260730_0007` 和专项测试 | 本地单元、静态迁移与离线 SQL 已通过；真实 PostgreSQL 17 触发器、推送 CI 和目标环境仍待验证，不能表述为真实取消/退款已接入 |
...
- 实现候选 `20ff71592096dfb4fc718cef050832a745bfe174` / 运行 `30534862434` 是 `0004` 历史基线；首次包含 `0005` 的运行 `30542366036` 暴露触发器字段访问缺陷，不能写成通过；`0006` 修正提交 `b8b8bea29477b472c942b7df40e8da6e9dbf05ab` / 运行 `30551146157` 已验证默认 `1738 passed, 39 deselected`、PostgreSQL 17 四文件 `15 passed`。当前 `0007` 需以第五个 PostgreSQL 文件和本次推送后的新运行结果为准；任何 CI 都不能替代目标环境迁移。
