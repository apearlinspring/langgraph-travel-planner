...
| 旅行社客户与门店权限 | 门店、角色授权、线下潜客、目标账户安全认领、固定告知读取、服务端规范化同意、激活/停用、主顾问分配和范围查询已实现；认领 token 为 256-bit 高熵、24 小时过期、可撤销、单次使用且数据库只存 SHA-256 摘要，同一旅行社同 target 待邀请唯一，同意记录只追加。客户拒绝/撤回同意或停用会原子收口内部交易。当前仍是应用层行级授权，不是 PostgreSQL RLS，且缺邀请投递、客户通知、真实身份核验、PII 档案、法律级同意、跨门店转移和门店停用/关闭 API | `0005` 明确区分 `legacy_direct` / `secure_claim` 和 `legacy_client_hash` / `server_canonical`；`0006` 修正共享延迟约束触发器的表专属 `NEW` 字段访问。legacy 升级认领会重置旧同意并先停用原 active 关系，之后必须重新 `grant`/激活。CI 已双绿，下一步补转店/关店并准备目标环境迁移、越权和并发复验 | P0 | [`b8b8bea`](https://github.com/apearlinspring/langgraph-travel-planner/commit/b8b8bea29477b472c942b7df40e8da6e9dbf05ab) / [Actions 30551146157](https://github.com/apearlinspring/langgraph-travel-planner/actions/runs/30551146157)：默认 `1738 passed, 39 deselected`，PostgreSQL 17 `15 passed`；目标环境仍待验证 |
| 旅行社人工取消与对账 | 旧 `cancellation_pending` / `manual_intervention` 已有状态但无退出入口；现新增取消案件、服务端暴露派生、专职审批、预订/财务分岗登记、不同审计人员核验、最新结果重做、只追加事件和数据库一致性门禁。当前只记录平台外人工结果，不执行真实供应商取消或退款 | `0007` 已在默认与 PostgreSQL 17 CI 双绿；下一步进入客户跨门店转移和门店 `closing/closed`。后续真实适配器必须绑定平台 Approval/HITL、回调验签、幂等、补偿和自动对账 | P0 | [`e17b97d`](https://github.com/apearlinspring/langgraph-travel-planner/commit/e17b97d82c24b7f5271973cc8f18e884124b7d6b) / [Actions 30602058425](https://github.com/apearlinspring/langgraph-travel-planner/actions/runs/30602058425)：默认 `1841 passed, 49 deselected`，PostgreSQL 17 `25 passed`；目标环境仍待验证 |
...
| M1 首部署 dry-run | `check_m1_first_deploy_dry_run.py` 已提供不连接服务器、不上传文件的预演入口，并会对脏工作树或缺目标输入 fail-closed；包含 `0007` 的当前候选尚缺绑定最终提交和真实目标输入的通过记录 | 冻结干净 commit，补齐目标输入后执行 dry-run，复核发布工具、Compose、公开边界和远端命令计划 | P0 | `scripts/check_m1_first_deploy_dry_run.py --json`、`docs/部署与运行/m1-first-deploy-dry-run.md` 和绑定候选的通过摘要 |
| M1 发布包 manifest | `build_release_artifact.py` 已能从干净 Git `HEAD` 生成 archive 和 manifest 并记录 commit、tree、tracked file count 与 sha256；包含 `0007` 的当前候选尚未生成与最终提交绑定的可复现候选发布包 | 冻结并评审干净 commit，生成不可变发布包，校验哈希并让部署记录引用同一 artifact（发布制品） | P0 | `scripts/build_release_artifact.py --execute --output-dir <release-output-dir> --json`、发布包 manifest 和哈希复核 |
...
- 实现候选 `20ff71592096dfb4fc718cef050832a745bfe174` / 运行 `30534862434` 是 `0004` 历史基线；首次包含 `0005` 的运行 `30542366036` 暴露触发器字段访问缺陷，不能写成通过；`0006` 修正提交 `b8b8bea29477b472c942b7df40e8da6e9dbf05ab` / 运行 `30551146157` 已验证默认 `1738 passed, 39 deselected`、PostgreSQL 17 四文件 `15 passed`。`0007` 实现提交 `e17b97d82c24b7f5271973cc8f18e884124b7d6b` / 运行 `30602058425` 已验证默认 `1841 passed, 49 deselected`、PostgreSQL 17 五文件 `25 passed`；任何 CI 都不能替代目标环境迁移。
