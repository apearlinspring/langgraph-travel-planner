...
| 旅行社客户与门店权限 | 门店、角色授权、线下潜客、目标账户安全认领、固定告知读取、服务端规范化同意、激活/停用、主顾问分配和范围查询已实现；认领 token 为 256-bit 高熵、24 小时过期、可撤销、单次使用且数据库只存 SHA-256 摘要，同一旅行社同 target 待邀请唯一，同意记录只追加。客户拒绝/撤回同意或停用会原子收口内部交易。当前仍是应用层行级授权，不是 PostgreSQL RLS，且缺邀请投递、客户通知、真实身份核验、PII 档案、法律级同意、跨门店转移和门店停用/关闭 API | `0005` 明确区分 `legacy_direct` / `secure_claim` 和 `legacy_client_hash` / `server_canonical`；`0006` 修正共享延迟约束触发器的表专属 `NEW` 字段访问。legacy 升级认领会重置旧同意并先停用原 active 关系，之后必须重新 `grant`/激活。下一步先取得修复后的 PostgreSQL CI 和目标环境迁移/越权/并发证据，再补客户获取、转店、合规和运营闭环 | P0 | 客户认领 token、notice 版本并发、服务/API、commit failure、迁移和数据库门禁测试；`0004` 的 [Actions 30534862434](https://github.com/apearlinspring/langgraph-travel-planner/actions/runs/30534862434) 仅作历史基线，不能证明 `0005 -> 0006` |
...
| M1 首部署 dry-run | `check_m1_first_deploy_dry_run.py` 已提供不连接服务器、不上传文件的预演入口，并会对脏工作树或缺目标输入 fail-closed；包含 `0005 -> 0006` 的当前候选尚缺绑定最终提交和真实目标输入的通过记录 | 冻结干净 commit，补齐目标输入后执行 dry-run，复核发布工具、Compose、公开边界和远端命令计划 | P0 | `scripts/check_m1_first_deploy_dry_run.py --json`、`docs/部署与运行/m1-first-deploy-dry-run.md` 和绑定候选的通过摘要 |
| M1 发布包 manifest | `build_release_artifact.py` 已能从干净 Git `HEAD` 生成 archive 和 manifest 并记录 commit、tree、tracked file count 与 sha256；包含 `0005 -> 0006` 的当前候选尚未生成与最终提交绑定的可复现候选发布包 | 冻结并评审干净 commit，生成不可变发布包，校验哈希并让部署记录引用同一 artifact（发布制品） | P0 | `scripts/build_release_artifact.py --execute --output-dir <release-output-dir> --json`、发布包 manifest 和哈希复核 |
...
| 2026-07-30 | 客户一致性触发器修正 | 首次包含 `0005` 的 PostgreSQL 17 CI 暴露共享触发器在错误表行类型上访问 `NEW.target_user_id` / `NEW.status`；不修改 frozen `0005`，新增 `0006` 按 `TG_TABLE_NAME` 分支后再访问表专属字段 | `20260730_0006` 迁移、revision-frozen helper 和迁移契约测试 | 本地迁移契约、编译及离线全链升降级已通过；修复后的 PostgreSQL CI 和目标环境迁移仍待验证 |
...
| 2026-07-30 | 旅行社客户安全认领与同意证据 | 新增 256-bit 高熵、24 小时过期、可撤销、单次使用且只存 SHA-256 摘要的目标账户认领；同一旅行社同 target 待邀请唯一。认证客户端先读取固定告知再回传预期版本/摘要；服务端生成只追加规范化证据。legacy 认领会重置旧同意并先停用原 active 关系；新激活/报价/订单要求 `secure_claim + server_canonical`，提交失败不得先返回 `2xx` | `0005` 功能提交已推送，`0006` 触发器修正待取得 CI 绿灯 | 不提供邀请投递、真实身份核验或法律合规证明；`0004` 历史 CI 不证明 `0005 -> 0006`，成功 CI 和目标环境证据必须另行记录 |
...
- 实现候选 `20ff71592096dfb4fc718cef050832a745bfe174` 曾在运行 `30534862434` 通过 3 个文件共 10 项 PostgreSQL 17 测试；这是 `0004` 历史基线。首次包含 `0005` 的运行 `30542366036` 暴露共享触发器字段访问缺陷，因此不能写成通过；`0006` 修正后必须重新触发数据库 job，且 CI 仍不能替代目标环境迁移。
