...
| CI/CD（持续集成/持续交付） 如何覆盖？ | 默认 CI 跑编译、知识库校验、测试收集、本地回归和前端验证；独立 PostgreSQL 17 job 当前应运行交易、客户生命周期、客户认领、门店权限和取消域五类场景。已确认的 `b8b8bea` / 运行 `30551146157` 默认 `1738 passed, 39 deselected`、PostgreSQL `15 passed` 只覆盖到 `0006`；`0007` 等待本次推送后的新结果。 | `.github/workflows/ci.yml`、`.github/workflows/staging-smoke.yml` | `uv run python -m pytest -q tests/test_ci_workflows.py` | CI 只证明一次性数据库路径，不能替代目标环境迁移、恢复、锁等待、人工操作真实性和供应商故障验证。 |
