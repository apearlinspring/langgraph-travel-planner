...
| CI/CD（持续集成/持续交付） 如何覆盖？ | 默认 CI 跑编译、知识库校验、测试收集、本地回归和前端验证；当前 PostgreSQL 17 job 候选加入转店/关店后共覆盖六类集成测试。历史 [`e17b97d`](https://github.com/apearlinspring/langgraph-travel-planner/commit/e17b97d82c24b7f5271973cc8f18e884124b7d6b) / [运行 30602058425](https://github.com/apearlinspring/langgraph-travel-planner/actions/runs/30602058425) 的默认 `1841 passed, 49 deselected`、PostgreSQL `25 passed` 只覆盖到 `0007`；`0008` 尚无可引用的 PostgreSQL CI 通过结果。 | `.github/workflows/ci.yml`、`.github/workflows/staging-smoke.yml` | `uv run python -m pytest -q tests/test_ci_workflows.py` | CI 只证明一次性数据库路径，不能替代目标环境迁移、恢复、并发锁等待、人工操作真实性和供应商故障验证。 |
