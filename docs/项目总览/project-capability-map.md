...
| CI/CD（持续集成/持续交付） 如何覆盖？ | 默认 CI 跑编译、知识库校验、测试收集、本地回归和前端验证；独立 PostgreSQL 17 job 运行交易、客户生命周期、客户认领和门店权限场景。实现候选 [`20ff715`](https://github.com/apearlinspring/langgraph-travel-planner/commit/20ff71592096dfb4fc718cef050832a745bfe174) 的 [运行 30534862434](https://github.com/apearlinspring/langgraph-travel-planner/actions/runs/30534862434) 曾得到默认 `1713 passed, 34 deselected` 和 PostgreSQL `10 passed`（3+5+2），但只覆盖 `0004`。首次包含 `0005` 的运行 `30542366036` 因共享触发器字段访问缺陷失败，`0006` 已修正但仍待新的成功运行。 | `.github/workflows/ci.yml`、`.github/workflows/staging-smoke.yml` | `.\.venv\Scripts\python -m pytest tests\test_ci_workflows.py -q` | `0005 -> 0006` 尚需新的成功 CI 结果；一次性 CI 数据库证据也不能替代目标环境迁移、恢复、锁等待和供应商故障验证。 |
