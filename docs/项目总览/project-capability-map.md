...
| 现在能算生产系统吗？ | 不能。截至 2026-07-03 的仓库外私有采样曾记录一次 M1 受控试运行部署、健康检查、数据库/缓存、恢复演练、短窗口探针和单轮 live chat 证据；这些是历史目标版本快照，不代表新增 `0005 -> 0006` 候选仍就绪。安全认领和服务端同意记录也只证明平台内控制面存在，不证明真实身份或法律合规。 | `docs/部署与运行/m1-controlled-trial-status.md`、`docs/部署与运行/production-readiness-gap.md`、`docs/架构与流程/agency-transaction-domain.md`、`docs/部署与运行/m1-controlled-trial-runbook.md` | `uv run python scripts\check_runtime_readiness.py --target production --json`；在目标版本和目标环境重新执行 migration review、release freeze、go/no-go、health/readiness 与 acceptance smoke | 当前公开状态应写成“目标业务是旅行社，客户与交易控制面已建立，历史 M1 证据可追溯，新增版本与真实交易均待验收”；不能承诺真实身份核验、法律合规、真实履约、长时间压测、多机高可用、PITR、异地灾备或供应商 SLA。 |

## 当前边界与改进路线

当前项目已经具备 Agent 应用骨架，以及旅行社门店、客户生命周期、顾问分配和交易数据控制面；仓库外留有截至 2026-07-03 的 M1 受控试运行历史证据。业务域覆盖多租户、门店授权、线下潜客、目标账户安全认领、固定告知读取、服务端规范化同意记录、关系激活/停用、主顾问分配、客户停用内部交易收口、产品、报价、订单、绑定式内部审核、事件、修订号、负载哈希和幂等；`0004` 用数据库触发器和统一锁序保护报价/订单绑定、合法迁移与并发授权，`0005` 增加认领/同意 provenance、同 target 待邀请唯一、终态不可变和新交易安全客户门禁，`0006` 修正共享延迟约束触发器的表专属字段访问。存量 `legacy_direct` / `legacy_client_hash` 不冒充安全认领或服务端证据：原账户仍可拒绝/撤回，升级认领会重置旧同意并先停用原 active 关系，再要求新的服务端 `grant` 和激活。旅行社 API 还会在响应发送前完成数据库提交，避免提交阶段失败却先返回 `2xx`。门店权限仍是应用层行级授权，不是 PostgreSQL RLS；批量导入、邀请投递、客户通知、真实身份核验、PII 档案与法律级同意、跨门店转移、门店停用/关闭、外部取消/退款与人工处理、平台 Approval 绑定、真实供应商预订、支付、补偿与对账尚未接入。`0005 -> 0006` 已由提交 `b8b8bea` / 运行 `30551146157` 得到默认 `1738 passed, 39 deselected` 和 PostgreSQL 17 `15 passed`；目标环境迁移和真实外部交易仍待验收。公开工程口径必须写成“目标业务是旅行社，安全认领与服务端同意控制面已建立，CI 证据可追溯，目标环境和真实外部交易仍待验收”。token 摘要、服务端同意记录、内部 `approved` 或内部 `cancelled` 都不代表真实身份、法律合规、真实库存、真实锁价、真实支付、供应商取消/退款、真实出票或供应链履约。
