...
| 旅行社客户生命周期做到哪一步？ | 20 个门店/客户操作覆盖门店授权、线下潜客、目标账户认领邀请、客户认领、固定技术告知读取、服务端同意、激活/停用、主顾问分配和只追加事件。认领凭证为 256-bit 高熵、24 小时过期、可撤销、单次使用且数据库只存 SHA-256 摘要；只有已登录目标账户能认领，同一旅行社同一目标同一时刻最多一条待处理邀请。认证客户端先读取告知 Markdown/版本/摘要/schema/渠道，提交预期版本/摘要防止 stale notice（过期告知），证据仍由服务端生成并只追加。`0006` 另行修正共享延迟约束触发器的表专属 `NEW` 字段访问。 | `app/models/agency_customer_lifecycle.py`、`app/models/agency_customer_identity.py`、`app/agency/customer_claim_service.py`、`app/agency/customer_consent.py`、`app/agency/customer_transaction_settlement.py`、`app/api/v1/agency_customers.py`、`alembic/versions/20260730_0005_agency_customer_invitation_claim.py`、`alembic/versions/20260730_0006_customer_claim_trigger_fix.py` | `uv run python -m pytest -q tests/test_customer_claim_tokens.py tests/test_agency_customer_claim_service.py tests/test_agency_customer_lifecycle_api.py tests/test_agency_customer_transaction_settlement.py tests/test_agency_customer_claim_migration.py` | 这是应用层行级授权，不是 PostgreSQL RLS；原始 token 只在首次签发提交成功的响应返回，幂等重放不返 token，丢失需撤销重发。legacy 认领会重置旧同意并先停用原 active 关系。仍没有批量导入、邀请投递/通知、真实身份核验、PII 档案、法律级同意、跨门店转移或门店停用/关闭 API。 |
...
| CI/CD（持续集成/持续交付） 如何覆盖？ | 默认 CI 跑编译、知识库校验、测试收集、本地回归和前端验证；独立 PostgreSQL 17 job 运行交易、客户生命周期、客户认领、门店权限和取消域五类场景。[`e17b97d`](https://github.com/apearlinspring/langgraph-travel-planner/commit/e17b97d82c24b7f5271973cc8f18e884124b7d6b) / [运行 30602058425](https://github.com/apearlinspring/langgraph-travel-planner/actions/runs/30602058425) 已验证默认 `1841 passed, 49 deselected`、PostgreSQL `25 passed`，覆盖到 `0007`。 | `.github/workflows/ci.yml`、`.github/workflows/staging-smoke.yml` | `uv run python -m pytest -q tests/test_ci_workflows.py` | CI 只证明一次性数据库路径，不能替代目标环境迁移、恢复、锁等待、人工操作真实性和供应商故障验证。 |
...
| 现在能算生产系统吗？ | 不能。截至 2026-07-03 的仓库外私有采样曾记录一次 M1 受控试运行部署、健康检查、数据库/缓存、恢复演练、短窗口探针和单轮 live chat 证据；这些是历史目标版本快照，不代表包含 `0007` 的当前候选仍就绪。安全认领和服务端同意记录也只证明平台内控制面存在，不证明真实身份或法律合规。 | `docs/部署与运行/m1-controlled-trial-status.md`、`docs/部署与运行/production-readiness-gap.md`、`docs/架构与流程/agency-transaction-domain.md`、`docs/部署与运行/m1-controlled-trial-runbook.md` | `uv run python scripts\check_runtime_readiness.py --target production --json`；在目标版本和目标环境重新执行 migration review、release freeze、go/no-go、health/readiness 与 acceptance smoke | 当前公开状态应写成“目标业务是旅行社，客户与交易控制面已建立，历史 M1 证据可追溯，新增版本与真实交易均待验收”；不能承诺真实身份核验、法律合规、真实履约、长时间压测、多机高可用、PITR、异地灾备或供应商 SLA。 |

## 当前边界与改进路线

当前项目已经具备 Agent 应用骨架，以及旅行社门店、客户生命周期、顾问分配和交易数据控制面；仓库外留有截至 2026-07-03 的 M1 受控试运行历史证据。业务域覆盖安全认领、服务端同意、报价、订单、绑定式内部审核，以及平台外人工取消结果的分岗登记与独立对账；`0004` 保护门店/交易绑定，`0005/0006` 保护安全认领和服务端证据，`0007` 增加只追加取消记录、案件/订单一致性、成员身份绑定冻结、按业务排除发起人/客户的合格审批员门禁、开放取消案送审互斥、逐业务撤权保护、取消审批人 active 资格复验、自我对账禁止和最新结果完成语义。旅行社 API 在响应发送前完成提交，避免延迟约束失败却先返回 `2xx`。门店权限整体仍是应用层行级授权，不是 PostgreSQL RLS；`0007` 的定向数据库门禁不等于全面 RLS。批量导入、邀请投递、真实身份核验、PII/法律合规、跨门店转移、门店关闭、平台 Approval 绑定、真实供应商/支付/退款/通知适配器、回调验签和跨系统补偿仍未接入。`e17b97d` / 运行 `30602058425` 已证明当前 `0007` 候选的默认与一次性 PostgreSQL 17 CI 路径，但目标环境仍待验证。公开口径必须写成“目标业务是旅行社，安全客户与交易控制面、平台外人工取消记录和独立核验已建立，真实外部交易仍默认关闭”。token 摘要、服务端同意记录、内部 `approved`、内部 `cancelled` 或取消案件 `completed` 都不代表真实身份、法律合规、真实库存、真实支付、供应商取消/退款、出票或履约。
