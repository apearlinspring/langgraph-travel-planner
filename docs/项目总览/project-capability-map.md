...
| CI/CD（持续集成/持续交付） 如何覆盖？ | 默认 CI 跑编译、知识库校验、测试收集、本地回归和前端验证；独立 PostgreSQL 17 job 精确运行交易、客户生命周期和门店权限 3 个文件，共 10 项测试（3+5+2），包括门店授权并发。旧 `319ac26` 只有 3 项交易测试绿灯，`0004` 必须由新提交重新验证。 | `.github/workflows/ci.yml`、`.github/workflows/staging-smoke.yml` | `.\.venv\Scripts\python -m pytest tests\test_ci_workflows.py -q` | 当前 10 项仍待新提交 CI 确认；一次性 CI 数据库不能替代目标环境迁移、恢复、锁等待和供应商故障验证。 |
