...
| 旅行社客户生命周期做到哪一步？ | 24 个门店/客户操作覆盖门店授权、线下潜客、安全认领、固定技术告知、服务端同意、激活/停用、主顾问分配、客户转店、门店清场/关闭和只追加事件。`0008` 允许 owner/admin 把 `active`、`inactive` 或 `blocked` 客户的当前门店立即原子转到同旅行社 active 门店，历史业务记录不改门店，`active` 客户可选目标主顾问。门店按 `active -> inactive -> closed`：`inactive` 停止新业务但保留清场动作；`closed` 不可逆，且要求当前客户（含 `inactive`/`blocked`）、待处理邀请、有效顾问分配/岗位授权、待审核订单、开放报价/订单/取消案件全部归零。 | `app/agency/customer_branch_transfer.py`、`app/agency/branch_administration.py`、`app/api/v1/agency_customers.py`、`app/models/agency_customer_lifecycle.py`、`app/models/agency_customer_identity.py`、`alembic/versions/20260731_0008_agency_branch_transfer_closure.py` | `uv run python -m pytest -q tests/test_agency_branch_transfer_closure_api.py tests/test_agency_branch_transfer_closure_service.py tests/test_agency_branch_transfer_closure_migration.py tests/test_branch_drain_authorization.py`；`c574649` / Actions `30606856484` 的 PostgreSQL 17 六文件 `34 passed` | 这是应用层行级授权，不是 PostgreSQL RLS。转店和关店不发送通知、不改写外部订单，也没有跨门店经理双边审批；批量导入、邀请投递、真实身份核验、PII 档案和法律级同意仍缺失。目标环境迁移/恢复和并发锁证据待补。 |
| 旅行社交易域做到哪一步？ | 已加入供应商产品、报价、订单、内部审核、取消案件、只追加人工结果、独立对账、订单事件、幂等和执行账本；13 个报价/订单操作外，另有 9 个取消操作，独立审计岗位可通过脱敏结果队列发现待办记录。`0007` 增加服务端暴露派生、取消期账本冻结、分岗登记、独立观察与最新结果完成语义；同时冻结 `agency_membership` 的 `id/agency_id/user_id/created_at` 身份绑定，要求提交订单和建立取消案时存在排除业务发起人与订单客户的 eligible approver（合格审批员），开放取消案阻止订单送审，并由数据库复验取消审批人的 active 旅行社、门店、成员与授权。锁序为 `customer -> branch/auth -> order -> payment/fulfillment -> case`。 | `app/models/agency_cancellation.py`、`app/agency/cancellation_service.py`、`app/agency/cancellation_support.py`、`app/api/v1/agency_cancellations.py`、`alembic/versions/20260730_0007_agency_cancellation_workflow.py` | `uv run python -m pytest -q tests/test_agency_cancellation_api.py tests/test_agency_cancellation_service.py tests/test_agency_cancellation_models.py tests/test_agency_cancellation_migration.py` | `approved` 仅为内部审核；取消 `completed` 可能表示无外部暴露订单经审批后直接内部取消，也可能表示各类最新成功人工结果已由不同审计人员核验为 `matched`。系统不调用供应商预订/取消、支付/退款或通知接口，原始回执、自动对账和跨系统补偿仍缺失。 |
| HITL 做到哪一步？ | 交易域已有确定性内部订单与取消审核：订单送审和取消建案前必须有排除发起人、订单客户的同门店合格审批员；待处理订单审核及 `approval_pending` 取消案撤权时按每笔业务保留合格替代审批员，不能只保留任意一名 `approver`。只有专职角色可决定，owner/admin 不能代替，客户或发起人不能自审。`inactive` 门店不能批准待审核订单，但可拒绝旧待办；取消申请、审核、结果登记、独立对账和恢复可在 `active`/`inactive` 门店继续完成清场。平台 Approval 仍只提供敏感动作记录、事件、角色权限和 readiness。 | `app/models/agency_order_review.py`、`app/agency/order_review_service.py`、`app/agency/cancellation_service.py`、`app/core/approval.py`、`app/api/v1/approvals.py`、`docs/治理与可观测/approval-governance.md` | `uv run python -m pytest -q tests/test_agency_order_review_service.py tests/test_agency_cancellation_service.py tests/test_branch_drain_authorization.py tests/test_approval_governance.py` | 内部审核没有绑定平台 `approval_request`，也没有 LangGraph `interrupt/resume`；通过或拒绝审核都不会执行供应商、支付、退款或通知动作。 |
...
| CI/CD（持续集成/持续交付） 如何覆盖？ | 默认 CI 跑编译、知识库校验、测试收集、本地回归和前端验证；PostgreSQL 17 job 加入转店/关店后共覆盖六类集成测试。[`c574649`](https://github.com/apearlinspring/langgraph-travel-planner/commit/c5746496203f628fe9a93a91ebb998c910c2a920) / [运行 30606856484](https://github.com/apearlinspring/langgraph-travel-planner/actions/runs/30606856484) 已验证默认 `1878 passed, 58 deselected`、PostgreSQL 六文件 `34 passed`，覆盖到 `0008`。 | `.github/workflows/ci.yml`、`.github/workflows/staging-smoke.yml` | `uv run python -m pytest -q tests/test_ci_workflows.py` | CI 只证明一次性数据库路径，不能替代目标环境迁移、恢复、并发锁等待、人工操作真实性和供应商故障验证。 |
...
| 现在能算生产系统吗？ | 不能。截至 2026-07-03 的仓库外私有采样曾记录一次 M1 受控试运行部署、健康检查、数据库/缓存、恢复演练、短窗口探针和单轮 live chat 证据；这些是历史目标版本快照，不代表包含 `0008` 的当前候选仍就绪。安全认领、服务端同意、转店和关店记录也只证明平台内控制面存在，不证明真实身份、法律合规、通知送达或外部订单变更。 | `docs/部署与运行/m1-controlled-trial-status.md`、`docs/部署与运行/production-readiness-gap.md`、`docs/架构与流程/agency-transaction-domain.md`、`docs/部署与运行/m1-controlled-trial-runbook.md` | `uv run python scripts\check_runtime_readiness.py --target production --json`；在目标版本和目标环境重新执行 migration review、release freeze、go/no-go、health/readiness 与 acceptance smoke | 当前公开状态应写成“目标业务是旅行社，客户与交易控制面已建立，历史 M1 证据可追溯，新增版本与真实交易均待验收”；不能承诺真实身份核验、法律合规、真实履约、长时间压测、多机高可用、PITR、异地灾备或供应商 SLA。 |

## 当前边界与改进路线

当前项目已经具备 Agent 应用骨架，以及旅行社门店、客户生命周期、顾问分配和交易数据控制面；仓库外留有截至 2026-07-03 的 M1 受控试运行历史证据。业务域覆盖安全认领、服务端同意、报价、订单、绑定式内部审核、平台外人工取消结果的分岗登记与独立对账，以及 `0008` 的当前客户转店和门店 `active -> inactive -> closed` 治理。转店由 owner/admin 立即原子执行，允许 `active`、`inactive`、`blocked` 客户迁移当前归属，历史门店业务记录不变；`inactive` 是停止新业务、继续清场的停业期，`closed` 是全部当前客户、待邀请、有效分配/授权、待审核和开放交易归零后的不可逆终态。旅行社 API 在响应发送前完成提交，避免延迟约束失败却先返回 `2xx`。门店权限整体仍是应用层行级授权，不是 PostgreSQL RLS；定向数据库门禁也不等于全面 RLS。批量导入、邀请投递、真实身份核验、PII/法律合规、跨门店经理双边审批、平台 Approval 绑定、真实供应商/支付/退款/通知适配器、回调验签和跨系统补偿仍未接入。`c574649` / 运行 `30606856484` 已证明到 `0008` 的默认与一次性 PostgreSQL 17 六文件 CI 路径；目标环境迁移/恢复和并发锁证据仍待补。公开口径必须写成“目标业务是旅行社，安全客户与交易控制面、平台内转店/关店治理和平台外人工取消记录已建立，真实外部交易仍默认关闭”。token 摘要、服务端同意记录、内部转店/关店、内部 `approved`、内部 `cancelled` 或取消案件 `completed` 都不代表真实身份、法律合规、通知送达、真实库存、真实支付、外部订单变更、供应商取消/退款、出票或履约。
...
> 当前项目不接真实供应链，但已经有目标账户安全认领、服务端规范化同意记录、门店范围的客户/顾问权限、客户停用时的内部交易原子收口、owner/admin 即时原子转店、`inactive` 清场和不可逆 `closed` 门店终态、确定性的旅行社内部订单审核，以及平台审批策略、审批请求和事件、工具审计、运行时观测、readiness 检查、CI 门禁和手动 staging smoke。转店保留历史门店记录；关店必须把当前客户（含 `inactive`/`blocked`）、待邀请、有效分配/授权、待审核和开放交易全部清零。认领与同意记录不等于真实身份或法律合规，转店/关店不发送通知也不修改外部订单，门店权限是应用层过滤而不是 PostgreSQL RLS。未来还要补跨门店经理双边审批，并在接真实支付、通知、预订或取消前，把平台审批记录与交易对象、LangGraph `interrupt/resume`、幂等、补偿和对账流程真正接通。
...
- `app/agency/customer_branch_transfer.py`
- `app/agency/branch_administration.py`
