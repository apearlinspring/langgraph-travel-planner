...
| 旅行社客户生命周期做到哪一步？ | 16 个门店/客户操作覆盖门店、门店岗位授权、线下潜客登记、关联已有用户、客户本人同意、激活/停用、主顾问分配/更换/结束和只追加事件。活跃客户拒绝/撤回同意或关系停用时，会原子结束当前顾问分配并收口内部报价/订单；旧 `pending_review` 明确拒绝前不能随客户关系重新激活。 | `app/models/agency_customer_lifecycle.py`、`app/agency/branch_authorization.py`、`app/agency/customer_lifecycle_service.py`、`app/agency/customer_transaction_settlement.py`、`app/api/v1/agency_customers.py`、`alembic/versions/20260726_0004_agency_customer_lifecycle.py` | `uv run python -m pytest -q tests/test_agency_customer_lifecycle_models.py tests/test_agency_customer_lifecycle_api.py tests/test_agency_customer_transaction_settlement.py` | 这是应用层行级授权，不是 PostgreSQL RLS；没有批量导入、安全邀请/认领 token、通知、姓名/电话/证件等 PII 档案、法律级同意、跨门店转移或门店停用/关闭 API；内部取消不代表供应商取消或退款。 |
...
| CI/CD（持续集成/持续交付） 如何覆盖？ | 默认 CI 跑编译、知识库校验、测试收集、本地回归和前端验证；独立 PostgreSQL 17 job 精确运行交易、客户生命周期和门店权限 3 个文件。实现候选 [`20ff715`](https://github.com/apearlinspring/langgraph-travel-planner/commit/20ff71592096dfb4fc718cef050832a745bfe174) 的 [运行 30534862434](https://github.com/apearlinspring/langgraph-travel-planner/actions/runs/30534862434) 已得到默认 `1713 passed, 34 deselected` 和 PostgreSQL `10 passed`（3+5+2），包括门店授权并发。 | `.github/workflows/ci.yml`、`.github/workflows/staging-smoke.yml` | `.\.venv\Scripts\python -m pytest tests\test_ci_workflows.py -q` | 这是一次性 CI 数据库证据，不能替代目标环境迁移、恢复、锁等待和供应商故障验证；后续数据库改动必须重跑。 |
...
| 现在能算生产系统吗？ | 不能。截至 2026-07-03 的仓库外私有采样曾记录一次 M1 受控试运行部署、健康检查、数据库/缓存、恢复演练、短窗口探针和单轮 live chat 证据；这些是历史目标版本快照，不代表新增 `0004` 候选仍就绪。新增客户与交易域也只证明模型、迁移和控制门禁存在。 | `docs/部署与运行/m1-controlled-trial-status.md`、`docs/部署与运行/production-readiness-gap.md`、`docs/架构与流程/agency-transaction-domain.md`、`docs/部署与运行/m1-controlled-trial-runbook.md` | `uv run python scripts\check_runtime_readiness.py --target production --json`；在目标版本和目标环境重新执行 migration review、release freeze、go/no-go、health/readiness 与 acceptance smoke | 当前公开状态应写成“目标业务是旅行社，客户与交易控制面已建立，历史 M1 证据可追溯，新增版本与真实交易均待验收”；不能承诺法律合规、真实履约、长时间压测、多机高可用、PITR、异地灾备或供应商 SLA。 |

## 当前边界与改进路线

当前项目已经具备 Agent 应用骨架，以及旅行社门店、客户生命周期、顾问分配和交易数据控制面；仓库外留有截至 2026-07-03 的 M1 受控试运行历史证据。业务域覆盖多租户、门店授权、线下潜客关联、本人同意、关系激活/停用、主顾问分配、客户停用内部交易收口、产品、报价、订单、绑定式内部审核、事件、修订号、负载哈希和幂等；`0004` 还用数据库触发器和统一锁序保护报价/订单绑定、合法迁移与并发授权。但门店权限仍是应用层行级授权，不是 PostgreSQL RLS；批量导入、安全邀请/认领 token、客户通知、PII 档案与法律级同意、跨门店转移、门店停用/关闭、外部取消/退款与人工处理、平台 Approval 绑定、真实供应商预订、支付、补偿与对账尚未接入。仓库也已具备资源申请、首部署 dry-run、发布包 manifest、服务器首部署、smoke/备份恢复/监控告警/事故回滚证据收集和 go/no-go 聚合入口。实现候选 `20ff71592096dfb4fc718cef050832a745bfe174` 已在运行 `30534862434` 得到默认 `1713 passed, 34 deselected` 和 PostgreSQL 17 的 `10 passed`；真实剩余缺口不是“缺 CI 脚本”，而是该候选尚未在目标环境完成同一时间窗的迁移、真实执行、复核与签核。公开工程口径必须写成“目标业务是旅行社，客户与交易控制面已建立，CI 证据与历史 M1 证据可追溯，目标环境和真实外部交易仍待验收”。站内模拟确认、同意哈希、内部 `approved` 或内部 `cancelled` 都不代表法律合规、真实库存、真实锁价、真实支付、供应商取消/退款、真实出票或供应链履约。
...
> 当前项目不接真实供应链，但已经有门店范围的客户/顾问权限、客户停用时的内部交易原子收口、确定性的旅行社内部订单审核，以及平台审批策略、审批请求和事件、工具审计、运行时观测、readiness 检查、CI 门禁和手动 staging smoke。门店权限是应用层过滤，不是 PostgreSQL RLS；内部取消和内部审核只改变业务状态，不是供应商取消/退款或完整 HITL 执行闭环。未来接真实支付、通知、预订或取消前，还要把平台审批记录与交易对象、LangGraph `interrupt/resume`、幂等、补偿和对账流程真正接通。
