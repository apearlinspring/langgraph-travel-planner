...
- `/api/v1/agency` 的交易子集覆盖报价草稿、发布、客户接受、订单草稿、提交审核和旅行社内部批准/拒绝，共 13 个操作、6 个 `POST`；门店与客户生命周期子集另有 20 个操作、12 个 `POST`；人工取消与对账子集另有 9 个操作、5 个 `POST`，覆盖取消申请、专职审批、平台外人工结果登记、脱敏结果队列、独立对账、恢复和脱敏事件查询。
- 门店权限是应用层行级授权，不是 PostgreSQL RLS（行级安全策略）。`owner`、`admin` 为旅行社全域角色；门店经理、顾问和审批员必须持有同一有效门店的有效授权，顾问还必须是客户当前主顾问。订单送审和取消建案前都必须有一名排除业务发起人、订单客户后的有效专职 `approver`，开放取消案件会阻止订单送审；待处理业务撤权时还必须逐笔保留合格替代审批员。只有该角色可以处理本门店审核。批准订单还要求客户保持 `active + granted`，客户停用后保留中的审核只能拒绝，且拒绝前不能重新激活客户关系；`owner`、`admin` 等角色不能代替，客户或审核发起人不能自审。
...
- `0005` 新增客户认领邀请、只追加同意记录和 provenance（来源）字段；`0006` 修正共享延迟约束触发器对不同表 `NEW` 字段的访问；当前 Alembic（数据库迁移工具）业务 head `0007` 新增人工取消、补偿记录和独立对账约束。取消案件 `requested_at` 记录案件创建，订单 `cancellation_requested_at` 仅在审批通过并进入订单取消处理时写入，订单 `cancelled_at` 只记录真正完成内部取消。最近已确认绿灯的 [`b8b8bea`](https://github.com/apearlinspring/langgraph-travel-planner/commit/b8b8bea29477b472c942b7df40e8da6e9dbf05ab) / [运行 30551146157](https://github.com/apearlinspring/langgraph-travel-planner/actions/runs/30551146157) 仍只覆盖 `0005 -> 0006`：默认 `1738 passed, 39 deselected`、PostgreSQL 17 `15 passed`；`0007` 必须使用本次推送后的新 CI 结果，且任何 CI-only（仅持续集成环境）证据仍不能替代目标环境迁移结论。
