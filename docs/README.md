...
- `0005` 新增客户认领邀请、只追加同意记录和 provenance（来源）字段，并用数据库门禁保护认领终态、同意记录不可修改及新交易的安全客户前置条件；`0006` 是当前 Alembic（数据库迁移工具）业务 head，只修正共享延迟约束触发器对不同表 `NEW` 字段的访问。`0005 -> 0006` 尚无可沿用的成功 PostgreSQL CI 或目标环境迁移结论；`0004` 的历史绿灯不能证明本次变更。
