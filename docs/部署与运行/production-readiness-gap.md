...
2. 再验证并扩展旅行社业务链路：`0005 -> 0006` 的历史绿灯不能覆盖 `0007`；先确认本次取消域默认与 PostgreSQL 17 CI，再到目标环境复验成员身份冻结、合格审批员排除、开放取消案送审冲突、逐业务撤权保护、审批人 active 资格、锁序、修订号、幂等、岗位分离、最新结果对账和只追加事件；随后补批量导入、邀请投递、真实身份与法律级同意、跨门店转移和门店关闭。
