...
- 门店隔离当前由应用服务和查询过滤器执行，不是 PostgreSQL RLS（行级安全策略）；客户关系也不含姓名、电话、证件等 PII（个人可识别信息）档案、邀请/认领、通知、跨门店转移或完整同意合规闭环。
...
| M1 受控试运行 | 使用真实环境和真实依赖，但只面向内部或少量白名单用户，人工兜底强。 | 2026-07-03 目标版本有一次历史就绪快照；新增 `0004` 候选仍须冻结、绑定 commit 并完整复验。 |
...
| 旅行社客户与交易域 | 已有租户、门店、成员、门店角色授权、客户生命周期、客户事件、主顾问分配、产品、报价、订单、内部审核、幂等和执行账本；客户 `deny/revoke` 或停用会原子结束当前分配并收口内部交易，`pending_review` 只能再拒绝且在拒绝前阻止客户关系重新激活；`0004` 触发器固化报价/订单绑定，复验跨表一致性和有效期，强制 `revision + 1`、合法状态迁移、外部动作关闭及订单/审核终态成对提交。实现候选 [`20ff715`](https://github.com/apearlinspring/langgraph-travel-planner/commit/20ff71592096dfb4fc718cef050832a745bfe174) 已在 [Actions 30534862434](https://github.com/apearlinspring/langgraph-travel-planner/actions/runs/30534862434) 得到默认 `1713 passed, 34 deselected` 和 PostgreSQL 17 `10 passed`（3+5+2）。 | 当前权限是应用层行级授权，不是 PostgreSQL RLS；还缺批量导入、带签名且可过期的一次性邀请/claim token、客户通知、PII 档案、法律级同意与撤回、客户跨门店转移、门店停用/关闭、供应商取消/退款、`cancellation_pending` 人工处理、平台 Approval 绑定、外部适配器、补偿和对账。10 项 CI 中已加入停用、重新同意和旧待审核阻断的数据库子场景，更广的取消状态矩阵仍由服务级测试覆盖；一次性 CI 数据库仍不是目标环境证据。 | 以已通过的隔离 PostgreSQL job 为仓库基线，在目标环境复验 `0001 -> 0002 -> 0003 -> 0004` 迁移、最小权限、锁等待和事务回滚；任何真实动作必须在 sandbox 通过回调重放、超时、重复请求和故障注入后再单项评估。 |
...
| 发布与回滚 | 有部署模板、本地/远端命令示例、发布候选冻结检查、发布包 manifest、服务器脚本和历史上线记录。 | 历史执行记录未绑定新增 `0004` 候选；候选冻结、回滚执行、迁移前备份和版本兼容仍需按本次提交确认。 | 每次发布有候选冻结记录、变更单、archive sha256、manifest、服务器脚本 dry-run / execute 摘要、回滚路径、迁移计划、验收摘要和负责人。 |
...
| `consent_evidence_hash` 已保存 | 已取得合法、有效且可举证的客户同意。 | 条款版本、身份核验、原始证据保全、撤回通知、留存期限和法务审查。 |
...
2. 再验证并扩展旅行社业务链路：以运行 `30534862434` 已通过的 3 个文件、10 项 `0004` PostgreSQL 集成测试为 CI 基线，在目标环境复验门店/客户范围、顾问分配、修订号冲突、同键异请求、四眼审核、授权并发和只追加事件，再补批量导入、安全邀请/认领、法律级同意、跨门店转移、门店关闭、供应商取消/退款和人工介入。
