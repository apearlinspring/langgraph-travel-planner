...
2. 再验证并扩展旅行社业务链路：`0005 -> 0006` 已在运行 `30551146157` 取得默认与 PostgreSQL 17 双绿，下一步到目标环境复验门店/客户范围、顾问分配、修订号冲突、同键异请求、四眼审核、授权并发和只追加事件；随后补批量导入、邀请投递、真实身份与法律级同意、跨门店转移、门店关闭、供应商取消/退款和人工介入。
