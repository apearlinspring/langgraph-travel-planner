...
| 旅行社客户与交易域 | 已有租户、门店、成员、门店角色授权、客户生命周期、客户事件、主顾问分配、产品、报价、订单、内部审核、幂等和执行账本；客户 `deny/revoke` 或停用会原子结束当前分配并收口内部交易，`pending_review` 只能再拒绝且在拒绝前阻止客户关系重新激活；`0004` 触发器固化报价/订单绑定，复验跨表一致性和有效期，强制 `revision + 1`、合法状态迁移、外部动作关闭及订单/审核终态成对提交。旧 `319ac26` 已在 PostgreSQL 17 CI 通过 3 项交易测试。 | 当前权限是应用层行级授权，不是 PostgreSQL RLS；还缺批量导入、带签名且可过期的一次性邀请/claim token、客户通知、PII 档案、法律级同意与撤回、客户跨门店转移、门店停用/关闭、供应商取消/退款、`cancellation_pending` 人工处理、平台 Approval 绑定、外部适配器、补偿和对账。旧 3 项 CI 早于 `0004`，本轮 10 项 PostgreSQL 集成测试（3 项交易、5 项客户生命周期、2 项门店权限）仍须在新提交上确认；其中已加入停用、重新同意和旧待审核阻断的数据库子场景，更广的取消状态矩阵仍由服务级测试覆盖。 | 先确认隔离 PostgreSQL job 精确运行交易、客户生命周期、门店权限 3 个测试文件并通过 `0001 -> 0002 -> 0003 -> 0004`、旧数据回填、门店授权、约束和并发测试，再在目标环境复验迁移、最小权限和事务回滚；任何真实动作必须在 sandbox 通过回调重放、超时、重复请求和故障注入后再单项评估。 |
...
2. 再验证并扩展旅行社业务链路：在隔离 PostgreSQL 精确执行 3 个文件的 10 项 `0004` 集成测试，复验门店/客户范围、顾问分配、修订号冲突、同键异请求、四眼审核、授权并发和只追加事件，再补批量导入、安全邀请/认领、法律级同意、跨门店转移、门店关闭、供应商取消/退款和人工介入。
