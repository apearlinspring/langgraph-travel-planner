...
| M1 受控试运行 | 使用真实环境和真实依赖，但只面向内部或少量白名单用户，人工兜底强。 | 2026-07-03 目标版本有一次历史就绪快照；包含 `0005` 的当前候选仍须冻结、绑定 commit 并完整复验。 |
...
| 旅行社客户与交易域 | 已有租户、门店、成员、门店角色授权、客户生命周期、认领邀请、只追加同意记录、客户事件、主顾问分配、产品、报价、订单、内部审核、幂等和执行账本。认领 token 为 256-bit 高熵、24 小时过期、可撤销、单次使用且数据库只存 SHA-256 摘要，只有已登录目标账户能认领，同一旅行社同 target 待邀请唯一；认证客户端读取固定告知并回传预期版本/摘要，规范化证据由服务端生成。`0005` 显式区分 `legacy_direct` / `secure_claim` 和 `legacy_client_hash` / `server_canonical`；legacy 升级认领会重置旧同意并先停用原 active 关系，新激活、报价和订单要求安全认领与服务端证据。旅行社 API 提交完成后才返回成功。 | 原始 token 只在首次签发提交成功后的响应返回，幂等重放不再返回；当前没有邀请投递。权限仍是应用层行级授权，不是 PostgreSQL RLS；还缺批量导入、客户通知、真实身份核验、PII 档案、法律级同意与撤回通知、客户跨门店转移、门店停用/关闭、供应商取消/退款、`cancellation_pending` 人工处理、平台 Approval 绑定、外部适配器、补偿和对账。`20ff715` / Actions `30534862434` 的 10 项 PostgreSQL 结果只覆盖 `0004`，不能证明 `0005`。 | 重新在隔离 PostgreSQL 验证 `0001 -> 0002 -> 0003 -> 0004 -> 0005`、非空 legacy 回填、token/同意记录不可变、notice 版本冲突、commit failure、同 target/并发认领、最小权限、锁等待和事务回滚，再在目标环境复验同一迁移链；任何真实动作必须在 sandbox 通过回调重放、超时、重复请求和故障注入后再单项评估。 |
...
| 发布与回滚 | 有部署模板、本地/远端命令示例、发布候选冻结检查、发布包 manifest、服务器脚本和历史上线记录。 | 历史执行记录未绑定新增 `0005` 候选；候选冻结、回滚执行、迁移前备份和版本兼容仍需按本次提交确认。 | 每次发布有候选冻结记录、变更单、archive sha256、manifest、服务器脚本 dry-run / execute 摘要、回滚路径、迁移计划、验收摘要和负责人。 |
...
2. 再验证并扩展旅行社业务链路：把运行 `30534862434` 的 3 个文件、10 项 `0004` PostgreSQL 测试仅作为历史基线，为 `0005` 新增安全认领、服务端同意、legacy 回填和并发/不可变门禁的新鲜 CI，再到目标环境复验门店/客户范围、顾问分配、修订号冲突、同键异请求、四眼审核、授权并发和只追加事件；随后补批量导入、邀请投递、真实身份与法律级同意、跨门店转移、门店关闭、供应商取消/退款和人工介入。
