...
当前项目适合作为旅行社 AI 应用工程样板和受控内部工作台：它已经具备状态机、RAG（检索增强生成）、MCP（模型上下文协议）工具、结构化 `report_data`、前端报告、readiness（就绪检查）、评估门禁、轻量观测证据，以及第一阶段旅行社门店、客户生命周期、顾问分配、报价、订单、事件和幂等控制面。

当前项目还不是完整生产系统。主要原因是：

- 交易域当前只完成数据与控制面骨架：客户拒绝/撤回同意或关系停用时可以原子收口内部报价/订单，但不会调用供应商取消或退款；真实供应商预订、取消、支付、退款、通知、出票、库存锁定和客服履约尚未接入，并由默认关闭的配置门禁阻断。
...
| M1 受控试运行 | 使用真实环境和真实依赖，但只面向内部或少量白名单用户，人工兜底强。 | 2026-07-03 目标版本有一次历史就绪快照；包含 `0005 -> 0006` 的当前候选仍须冻结、绑定 commit 并完整复验。 |
...
| 旅行社客户与交易域 | 已有租户、门店、成员、门店角色授权、客户生命周期、认领邀请、只追加同意记录、客户事件、主顾问分配、产品、报价、订单、内部审核、幂等和执行账本。认领 token 为 256-bit 高熵、24 小时过期、可撤销、单次使用且数据库只存 SHA-256 摘要，只有已登录目标账户能认领，同一旅行社同 target 待邀请唯一；认证客户端读取固定告知并回传预期版本/摘要，规范化证据由服务端生成。`0005` 显式区分 `legacy_direct` / `secure_claim` 和 `legacy_client_hash` / `server_canonical`；legacy 升级认领会重置旧同意并先停用原 active 关系，新激活、报价和订单要求安全认领与服务端证据。`0006` 修正共享延迟约束触发器的表专属 `NEW` 字段访问。旅行社 API 提交完成后才返回成功。 | 原始 token 只在首次签发提交成功后的响应返回，幂等重放不再返回；当前没有邀请投递。权限仍是应用层行级授权，不是 PostgreSQL RLS；还缺批量导入、客户通知、真实身份核验、PII 档案、法律级同意与撤回通知、客户跨门店转移、门店停用/关闭、供应商取消/退款、`cancellation_pending` 人工处理、平台 Approval 绑定、外部适配器、补偿和对账。`20ff715` / Actions `30534862434` 的 10 项 PostgreSQL 结果只覆盖 `0004`，不能证明 `0005 -> 0006`。 | 重新在隔离 PostgreSQL 验证 `0001 -> 0002 -> 0003 -> 0004 -> 0005 -> 0006`、非空 legacy 回填、token/同意记录不可变、notice 版本冲突、commit failure、同 target/并发认领、最小权限、锁等待和事务回滚，再在目标环境复验同一迁移链；任何真实动作必须在 sandbox 通过回调重放、超时、重复请求和故障注入后再单项评估。 |
| Agent 高风险动作 | 内部审核已落实订单门店专职 `approver`、禁止自审、客户/订单/审核行锁、修订号和业务快照绑定；授权写会持有门店/成员共享锁，防止并发撤权造成 TOCTOU（检查与使用时序差）竞态。平台审批仍有持久化记录和只追加事件骨架。 | 内部 `approved` 没有绑定平台 `approval_request`，也没有 LangGraph `interrupt/resume`、checkpoint 回写或外部动作恢复。 | 所有真实支付、预订、取消、退款、通知和客户资料导出动作继续默认禁止；上线前必须另行完成平台 HITL（人类在环）绑定、权限、幂等、补偿和审计。 |
...
| 发布与回滚 | 有部署模板、本地/远端命令示例、发布候选冻结检查、发布包 manifest、服务器脚本和历史上线记录。 | 历史执行记录未绑定新增 `0005 -> 0006` 候选；候选冻结、回滚执行、迁移前备份和版本兼容仍需按本次提交确认。 | 每次发布有候选冻结记录、变更单、archive sha256、manifest、服务器脚本 dry-run / execute 摘要、回滚路径、迁移计划、验收摘要和负责人。 |
...
| 交易运营闭环 | 有支付尝试、履约记录和订单事件的数据结构。 | 缺供应商对账、财务清分、取消/退款/改签、部分履约、合同/发票、客服工单和服务质量反馈。 | 选一个受控产品完成从报价、审核、沙箱支付、沙箱预订到取消/补偿和对账的可重放验收。 |
...
2. 再验证并扩展旅行社业务链路：把运行 `30534862434` 的 3 个文件、10 项 `0004` PostgreSQL 测试仅作为历史基线，为 `0005 -> 0006` 的安全认领、服务端同意、legacy 回填、共享触发器修正和并发/不可变门禁取得新鲜 CI，再到目标环境复验门店/客户范围、顾问分配、修订号冲突、同键异请求、四眼审核、授权并发和只追加事件；随后补批量导入、邀请投递、真实身份与法律级同意、跨门店转移、门店关闭、供应商取消/退款和人工介入。
