...
| 旅行社租户、门店与客户 | `agency`、`agency_membership`、`agency_branch`、`agency_branch_lifecycle_event`、`agency_branch_role_grant`、`agency_customer`、`agency_customer_branch_transfer`、`agency_customer_invitation`、`agency_customer_consent_record`、`agency_customer_event`、`agency_customer_advisor_assignment` | 旅行社、门店、成员授权、目标账户安全认领、只追加同意记录、客户生命周期、当前门店转移、门店停用/关闭和主顾问分配；门店范围由应用层执行，不是 PostgreSQL RLS（行级安全策略）。 |
...
`0004` 历史候选运行原 3 个文件共 `10 passed`；`0005 -> 0006` workflow 增加客户认领文件，运行 `30551146157` 得到四文件 `15 passed`（3+5+5+2）。workflow 后续增加 `tests/test_agency_cancellation_postgres_integration.py`，覆盖 `0007` 升降级、订单/案件提交时一致性、只追加结果、四眼与自我对账拒绝、最新结果完成语义和外部动作关闭；运行 `30602058425` 得到五文件 `25 passed`（3+5+5+2+10），同一运行默认 job 为 `1841 passed, 49 deselected`。六文件运行 `30606856484` 已验证 `0008` 的 `inactive` 清场、历史门店保留、转店阻断、最终关闭和直接 SQL 门禁，结果为 `34 passed`（3+5+5+2+10+9），同一运行默认 job 为 `1878 passed, 58 deselected`。复杂业务数据下的失败关闭 downgrade、并发 lock-wait 和目标环境迁移仍需单独补证据。CI 不得读取 staging 或 production 数据库 secret（密钥配置）。
...
| CI（持续集成） | 一次性 PostgreSQL service 中执行 `upgrade head -> downgrade base -> legacy upgrade head` | 静态 job 运行 development readiness；数据库 job 精确运行交易、客户生命周期、客户认领、门店权限、取消域与转店/关店六个 PostgreSQL 测试文件。`c574649` / 运行 `30606856484` 已验证默认 `1878 passed, 58 deselected` 与 PostgreSQL 17 六文件 `34 passed`，覆盖到 `0008`。 | 仅使用固定 CI-only 凭据和隔离数据库，不读取部署环境 secret；CI 不能替代目标环境迁移验收、恢复演练或并发锁等待证据。 |
