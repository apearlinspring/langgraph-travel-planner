...
| CI（持续集成） | 一次性 PostgreSQL service 中执行 `upgrade head -> downgrade base -> legacy upgrade head` | 静态 job 运行 development readiness；当前候选数据库 job 精确运行交易、客户生命周期、客户认领、门店权限、取消域与转店/关店六个 PostgreSQL 测试文件。`e17b97d` / 运行 `30602058425` 已验证默认 `1841 passed, 49 deselected` 与 PostgreSQL 17 五文件 `25 passed`，只覆盖到 `0007`；`0008` 六文件配置尚无 PostgreSQL CI 通过记录。 | 仅使用固定 CI-only 凭据和隔离数据库，不读取部署环境 secret；CI 不能替代目标环境迁移验收。 |
