...
This option runs the image/audio/video ingestion and retrieval acceptance under `.runtime`. It requires a real `DASHSCOPE_API_KEY`, usable `ffmpeg` / `faster-whisper`, and prepared local sample files. Default CI should not enable it.
