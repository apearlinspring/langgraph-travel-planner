...
知行旅行规划器是一个面向旅行规划场景的多智能体系统。后端使用 FastAPI（快速应用接口框架），主对话流程由 LangGraph（图式智能体编排框架）和 LangChain（大模型应用编排框架）协调，通过 RAG（检索增强生成）补充本地知识，通过 MCP（模型上下文协议）接入可选外部能力，并使用 PostgreSQL（关系型数据库）和 Redis（缓存数据库）保存业务状态。
