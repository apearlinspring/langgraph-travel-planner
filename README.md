...
- 当前业务迁移 head 为 `0008`：新增客户转店记录、门店生命周期事件和 `closed_at`，把客户关联外键改为稳定的旅行社/客户身份，使历史邀请、同意、事件、分配和交易继续保留发生时门店；数据库门禁约束转店绑定、门店修订号、只追加记录和 `closed` 终态。存在转店/关店业务数据或历史子记录门店已与客户当前门店不同后，降级会失败关闭。`0008` 当前只有本工作树的单元、API 和静态迁移契约证据，PostgreSQL 17 CI、目标环境迁移/恢复和并发锁等待仍待验证。
