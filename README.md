...
- `/api/v1/agency` 的报价、订单和审核子集为 13 个操作，其中 6 个 `POST` 强制 `Idempotency-Key`（幂等键）；门店与客户生命周期子集为 20 个操作，其中 12 个 `POST` 强制该幂等键；人工取消与对账子集另有 9 个操作，其中 5 个 `POST` 同样要求幂等键和预期修订号，审计岗位可从脱敏人工结果队列取得待核验记录 ID 与当前对账状态。
- 客户把订单提交为 `pending_review` 或创建取消案件前，门店必须存在至少一名排除业务发起人和订单客户后的有效专职 `approver`；开放取消案件会阻止同一订单进入审核。待处理订单审核或 `approval_pending` 取消案件存在时，撤权操作必须为每笔业务保留一名同样合格的替代审批员。只有同一有效门店中拥有有效成员身份和授权的专职 `approver` 可读取已生成审核记录的订单快照并处理审核；批准还要求客户保持 `active + granted`，客户停用后保留中的审核只能拒绝为 `review_rejected`，且拒绝前客户关系不能重新激活。该只读权限不包含未提交订单或报价创建/发布，`owner`、`admin` 等角色也不能代替审批员，客户或审核发起人不能自审。
...
- `0006` 只修正 `0005` 共享延迟约束触发器对不同表 `NEW` 字段的访问方式；`0005` frozen 迁移保持不变。当前迁移 head 为 `0007`：新增取消案件、只追加案件事件、人工补偿结果和独立对账表，并以 PostgreSQL 触发器约束复合绑定、`revision + 1`、状态机、合格审批员、只追加记录、最新结果对账和外部动作恒为关闭；同时冻结 `agency_membership` 的身份绑定，防止改绑账户绕过审批隔离。案件的 `requested_at` 记录取消申请创建时间；订单的 `cancellation_requested_at` 仅在申请获批并进入订单取消处理时写入；订单的 `cancelled_at` 只在订单真正进入内部 `cancelled` 时写入。无外部暴露的直接完成路径中，后两个时间可以同为审批时间；拒绝申请不会改写这两个订单时间。
...
- 当前没有邀请投递/客户通知、真实身份核验、法律级同意证据、客户 PII 档案、跨门店转移或门店关闭工作流，也没有真实供应商预订/取消、库存锁定、收款/退款、出票、酒店确认、供应商自动对账、财务清分和跨系统完整失败补偿；现有取消闭环只记录平台外人工结果并做平台内独立核验。
...
数据库名必须包含独立的 `test` 或 `ci` 段；测试会创建并删除随机 schema（数据库命名空间），不得连接 staging（预生产）或 production（生产）数据库。当前 workflow 已加入第五个取消域文件；`0007` 候选仍须以推送后的 GitHub Actions 结果为准。实现候选 [`20ff715`](https://github.com/apearlinspring/langgraph-travel-planner/commit/20ff71592096dfb4fc718cef050832a745bfe174) 的 [GitHub Actions 运行 30534862434](https://github.com/apearlinspring/langgraph-travel-planner/actions/runs/30534862434) 曾在 `0005` 之前精确执行原三个文件并得到 `10 passed`（3 项交易、5 项客户生命周期、2 项门店权限），同一运行的默认 job 为 `1713 passed, 34 deselected`，这是 `0004` 历史基线。当前已确认的最近绿灯仍是 [`b8b8bea`](https://github.com/apearlinspring/langgraph-travel-planner/commit/b8b8bea29477b472c942b7df40e8da6e9dbf05ab) 的 [GitHub Actions 运行 30551146157](https://github.com/apearlinspring/langgraph-travel-planner/actions/runs/30551146157)：默认 `1738 passed, 39 deselected`，PostgreSQL 17 四个文件 `15 passed`（3 项交易、5 项客户生命周期、5 项客户认领、2 项门店权限）。该历史结果只证明 `0005 -> 0006` 在一次性 CI 数据库中的迁移和约束路径，不能替代 `0007`、目标环境迁移、恢复、锁等待或真实外部依赖验证。需要真实 LLM（大语言模型）、MCP 服务或外部 API 的其他集成测试仍单独标记，不属于默认快速回归。
