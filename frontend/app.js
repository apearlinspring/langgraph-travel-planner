// === 逻辑代码保持不变，仅适配样式类名 ===
      ...
      function renderJourneyDayInsight(entry) {
        ...
        const routeRailTitle = entry.shell.querySelector(".journey-map-stage-rail-title");
        const routeRailList = entry.shell.querySelector(".journey-map-stage-rail-list");
        ...
        if (activeDayKey === "all") {
          ...
          insightList.innerHTML = entry.routeStops
            ...
            .join("");
          if (routeRailTitle && routeRailList) {
            routeRailTitle.textContent = "总览路线顺序";
            routeRailList.innerHTML = entry.routeStops
              .filter((item) => item.value)
              .map(
                (stop, index) => `
                  <button
                    class="journey-map-stage-stop"
                    type="button"
        ...
        if (routeRailTitle && routeRailList) {
          routeRailTitle.textContent = `${selectedLayer?.label || "当日"}路线顺序`;
          routeRailList.innerHTML = waypoints
            .slice(0, 6)
            .map(
              (point, index) => `
                <button
      ...
      function extractTitleDays(text = "") {
        const match = text.match(/(\d+\s*天\s*\d+\s*[晚夜]|三天两夜|两天一夜|五天四夜|四天三晚)/);
      ...
      function generateConversationTitle(text = "") {
        ...
        const routeMatch = normalized.match(
          /从\s*([^，。；、\s]{1,12})\s*(?:出发)?\s*(?:到|去)\s*([^，。；、\s]{1,12})/
        );
        const destinationMatch = normalized.match(
          /(?:去|到)\s*([^，。；、\s]{1,12})(?:玩|旅游|旅行|住|看|逛|过|，|。|\s)/
        );
        const dayText = extractTitleDays(normalized);
        const styleTag = /情侣/.test(normalized)
          ? "情侣"
          : /亲子/.test(normalized)
          ? "亲子"
          : /美食/.test(normalized)
        ...
        if (routeMatch) {
          ...
          return [origin && destination ? `${origin}去${destination}` : destination, dayText, styleTag]
            .filter(Boolean)
            .join("·")
            .slice(0, 24);
        }
        ...
        if (destinationMatch?.[1]) {
          ...
          return [destination, dayText, styleTag].filter(Boolean).join("·").slice(0, 24);
        }

        return normalized
          .replace(/^(我想|帮我|请帮我|麻烦帮我|想要|计划)/, "")
          .split(/[。！？]/)[0]
          .trim()
          .slice(0, 24) || DEFAULT_CONVERSATION_TITLE;
      }
      ...
      async function maybeAutoNameCurrentConversation(text = "") {
        ...
        try {
          await updateConversationTitle(current.id, nextTitle, { silent: true });
        } catch (error) {
          console.error(error);
        }
      }
      ...
      function sanitizeConversationTitleSegment(value = "") {
        return (value || "")
          .replace(/^(从|去|到|往|想去|准备去)\s*/, "")
          .replace(/\s*(出发|游玩|旅行|旅游|看看|逛逛|住几晚|玩几天)\s*$/, "")
          .replace(/[，。；、,.!！?？]+/g, " ")
          .replace(/\s+/g, " ")
          .trim();
      }
      ...
      function extractTitleDays(text = "") {
        ...
        const match = normalized.match(
          /(\d+天\d+[晚夜]|一天一夜|两天一夜|三天两夜|四天三夜|五天四夜|六天五夜|七天六夜)/
        );
      ...
      function generateConversationTitle(text = "") {
        ...
        if (!normalized) return DEFAULT_CONVERSATION_TITLE;
        const routeMatch = normalized.match(
          /从\s*([^\s，。；、]{1,12})\s*(?:出发)?\s*(?:去|到)\s*([^\s，。；、]{1,12})/
        );
        const destinationMatch = normalized.match(
          /(?:去|到)\s*([^\s，。；、]{1,12})(?:旅游|旅行|游玩|玩|逛|看看)?/
        );
        const dayText = extractTitleDays(normalized);
        const styleTag = /情侣/.test(normalized)
          ? "情侣"
        ...
        return (
          normalized
            .replace(/^(我想|帮我|请帮我|麻烦帮我|想要|计划|准备)\s*/, "")
            .split(/[。！？!?]/)[0]
            .trim()
            .slice(0, 24) || DEFAULT_CONVERSATION_TITLE
        );
      ...
      function sanitizeConversationTitleSegment(value = "") {
        return (value || "")
          .replace(/^(?:\u4ece|\u53bb|\u5230|\u5f80|\u60f3\u53bb|\u51c6\u5907\u53bb)\s*/u, "")
          .replace(
            /\s*(?:\u51fa\u53d1|\u6e38\u73a9|\u65c5\u884c|\u65c5\u6e38|\u770b\u770b|\u901b\u901b|\u4f4f\u51e0\u665a|\u73a9\u51e0\u5929)\s*$/u,
            ""
          )
          .replace(/[，。；、,.!！?？]+/gu, " ")
          .replace(/\s+/g, " ")
      ...
      function extractTitleDays(text = "") {
        const normalized = (text || "").replace(/\s+/g, "");
        const match = normalized.match(
          /(\d+\u5929\d+[\u665a\u591c]|\u4e00\u5929\u4e00\u591c|\u4e24\u5929\u4e00\u591c|\u4e09\u5929\u4e24\u591c|\u56db\u5929\u4e09\u591c|\u4e94\u5929\u56db\u591c)/u
        );
        return match ? match[1] : "";
      }
      ...
      function extractJourneyCityPair(text = "") {
        const normalized = text.replace(/\s+/g, " ").trim();
        const cleanCitySegment = (value = "", edge = "end") => {
          const parts = value
            .split(/[！!：:，,。；;、]/)
            .map((part) => part.trim())
            .filter(Boolean);
          const picked = edge === "start" ? parts[0] : parts[parts.length - 1] || value;
          return picked
            .replace(/^(行程概览|旅行计划|方案概览|路线|主路线)\s*[：:]?\s*/g, "")
            .replace(/^(需求很完整|信息基本齐了|现在先把|我已经按你的要求查到|我先帮你梳理确认一下)\s*/g, "")
        ...
        for (const pattern of patterns) {
          ...
          if (match) {
            return {
              origin: cleanCitySegment(match[1], "end"),
              destination: cleanCitySegment(match[2], "start"),
            };
          }
        }
      ...
      function extractJourneyPrimaryDestination(text = "") {
        const normalized = text.replace(/\s+/g, " ").trim();
        const patterns = [
          /去\s*([^，。；、\s]{1,12}?)(?:玩|住|看|逛|过|待|，|。|\s)/,
          /到\s*([^，。；、\s]{1,12}?)(?:玩|住|看|逛|过|待|，|。|\s)/,
          /([^，。；、\s]{1,12}?)是个(?:很|不|挺|真|特别|非常)/,
          /推荐(?:去|住)?\s*([^，。；、\s]{1,12}?)(?:，|。|\s)/,
        ];
        for (const pattern of patterns) {
          const match = normalized.match(pattern);
      ...
      function buildJourneyPreviewState(summaryBlocks, sections) {
        const combinedText = [...summaryBlocks.flat(), ...sections.flatMap((section) => section.rawLines)]
          .join(" ")
          .replace(/\s+/g, " ")
          .trim();
        const rawCityPair = extractJourneyCityPair(combinedText);
        const originFallback = extractJourneyPrimaryOrigin(combinedText);
        const destinationFallback = extractJourneyPrimaryDestination(combinedText);
        const cityPair =
          rawCityPair ||
        ...
        const destinationSection =
          sections.find((section) => section.tone === "overview") || {
            tone: "overview",
            title: destinationFallback || cityPair?.destination || "待确认目的地",
            rawLines: splitJourneyFragments(summaryBlocks.flat().join(" ")).slice(0, 3),
          };
        ...
        const hasConcreteRoutePayload =
          (
            ...
            /高铁|火车|自驾|大巴|公交|航班|车程|打车/.test(combinedText)
          ) &&
          (
            sections.some((section) => ["overview", "stay"].includes(section.tone)) ||
            /住宿|酒店|民宿|温泉|景点|玩法|太白山|眉县/.test(combinedText)
          );
        const shouldRender =
      ...
      function renderJourneyAtlas(
        ...
        const mapPayload = serializeMapPayload({
          origin: cityPair?.origin || previewStops[0]?.value || "",
          destination: cityPair?.destination || previewStops[1]?.value || "",
        ...
        const atlasTitle =
          cityPair?.origin && cityPair?.destination
            ? `${cityPair.origin} → ${cityPair.destination}`
            : cityPair?.destination || previewStops[1]?.value || "这段旅程";
        ...
        return `
          <section class="journey-map-studio">
            ...
            <div
              ...
              data-route-stops="${serializeMapPayload(routeStops)}"
            >
              <div class="journey-live-map-head">
                <div class="journey-live-map-head-copy">
                  <span class="journey-map-shell-kicker">
                    <i class="fa-solid fa-earth-asia"></i> 地图路线工作区
                  </span>
                  <strong>${escapeHtml(atlasTitle)}</strong>
                  <span>${
                    hasDayView
                      ? "你可以切到 Day 1 / Day 2 单独看某天，也可以把某一天突出显示，其他天自动压淡。"
                      : "当前先把主路线和关键节点定住，等回复里出现更完整的 Day 结构后，这里会自动切换成分日模式。"
                  }</span>
                </div>
                <div class="journey-live-map-toolbar">
                  <div class="journey-live-map-toolbar-row">
                    <div class="journey-live-map-actions">
                      <button class="journey-map-action-btn active" type="button" data-map-action="route">
                        主路线
                      </button>
                      <button class="journey-map-action-btn" type="button" data-map-action="highlights">
                        看点
                      </button>
                      <button class="journey-map-action-btn secondary" type="button" data-map-action="expand">
                        放大查看
                      </button>
                    </div>
                    <div class="journey-live-map-styles">
                      <button class="journey-map-style-btn active" type="button" data-map-style="standard">
                        标准
                      </button>
                      <button class="journey-map-style-btn" type="button" data-map-style="terrain">
                        地形感
                      </button>
                      <button class="journey-map-style-btn" type="button" data-map-style="calm">
                        清爽
                      </button>
                    </div>
                  </div>
              ...
              <div class="journey-live-map-shell-body journey-live-map-shell-body--studio">
                <aside class="journey-map-sidebar">
                  <div class="journey-map-sidebar-card intro">
                    ...
                    <div class="journey-map-sidebar-copy">
                      ...
                      <p>${
                        hasDayView
                          ? "左侧先切日程，右侧主画布负责看路线、落脚点和沿途看点。"
                          : "现在先把出发、目的地、交通和落脚点稳定下来，后面再继续展开成完整路线。"
                      }</p>
                    </div>
                  </div>
                  <div class="journey-map-sidebar-card">
                    <div class="journey-map-sidebar-head">
                      <span>分日切换</span>
                      <strong>${hasDayView ? "总览 / 单日" : "等待分日"}</strong>
                    </div>
                    <div class="journey-map-day-stack">
                      <button
                        class="journey-map-day-btn journey-map-day-card overview active"
                        type="button"
                        data-map-day="all"
                      >
                        <div class="journey-map-day-card-head">
                          <span class="journey-day-label">总览</span>
                          <span class="journey-map-day-card-meta">${escapeHtml(
                            `${routeStops.filter((item) => item.value && !/待/.test(item.value)).length} 个节点`
                          )}</span>
                        </div>
                        <strong>${escapeHtml(atlasTitle)}</strong>
                        <div class="journey-map-day-card-route">
                          ${escapeHtml(
                            routeStops
                              .map((item) => item.value)
                              .filter((item) => item && !/待/.test(item))
                        ...
                        hasDayView
                          ? dayPlans
                              .map(
                                (day, index) => `
                                  <button
                                    class="journey-map-day-btn journey-map-day-card"
                                    type="button"
                                    data-map-day="${escapeHtml(day.key)}"
                                  >
                                    <div class="journey-map-day-card-head">
                                      <span class="journey-day-label">${escapeHtml(
                                        day.label || `Day ${index + 1}`
                                      )}</span>
                                      <span class="journey-map-day-card-meta">${escapeHtml(
                                        `${day.waypoints.length || 0} 个节点`
                                      )}</span>
                                    </div>
                                    <strong>${escapeHtml(day.title || `${day.label} 行程`)}</strong>
                                    <div class="journey-map-day-card-route">
                                      ${escapeHtml(
                                        day.waypoints.join(" · ") || "等待补齐当日路线"
                                      )}
                                    </div>
                                  </button>
                                `
                              ...
                              .join("")
                          : `
                              <div class="journey-map-sidebar-note">
                                等回复里出现更完整的 Day 1 / Day 2 结构后，这里会自动切到按天查看。
                              </div>
                            `
                      }
                    </div>
                    ${
                      hasDayView
                        ? `
                            <div class="journey-live-map-day-modes journey-live-map-day-modes--studio">
                              <button class="journey-map-day-mode-btn active" type="button" data-map-day-mode="solo">单独显示</button>
                              <button class="journey-map-day-mode-btn" type="button" data-map-day-mode="fade">突出显示</button>
                            </div>
                          `
                        : ""
                    }
                    <div class="journey-map-floating-actions">
                      <button class="journey-map-action-btn active" type="button" data-map-action="route">主路线</button>
                      <button class="journey-map-action-btn" type="button" data-map-action="highlights">看点</button>
                      <button class="journey-map-action-btn secondary" type="button" data-map-action="expand">放大查看</button>
                    </div>
                    <div class="journey-map-floating-actions journey-live-map-styles">
                      <button class="journey-map-style-btn active" type="button" data-map-style="standard">标准</button>
                      <button class="journey-map-style-btn" type="button" data-map-style="terrain">地形感</button>
                      <button class="journey-map-style-btn" type="button" data-map-style="calm">清爽</button>
                    </div>
                  </div>
                  <div class="journey-map-sidebar-card">
                    <div class="journey-map-sidebar-head">
                        ...
                        .map(
                          (stop) => `
                            ...
                            >
                              ...
                              <strong class="journey-map-route-stop-value">${escapeHtml(
                                stop.value
                              )}</strong>
                              <span class="journey-map-route-stop-note">${escapeHtml(
                                stop.note
                              )}</span>
                            </button>
                          `
                ...
                <div class="journey-map-stage">
                  <div class="journey-map-floating-controls">
                    <div class="journey-map-floating-group journey-map-floating-group--days">
                      <span class="journey-map-floating-label">分日切换</span>
                      <div class="journey-map-floating-row">
                        <button class="journey-map-day-btn active" type="button" data-map-day="all">
                          <span>总览</span>
                          <small>${escapeHtml(
                            `${routeStops.filter((item) => item.value && !item.disabled).length} 站`
                          )}</small>
                        </button>
                        ${
                          hasDayView
                                ...
                                .map(
                                  (day, index) => `
                                    ...
                                    >
                                      ...
                                      <small>${escapeHtml(`${day.waypoints.length || 0} 站`)}</small>
                                    </button>
                                  `
                                )
                                .join("")
                    ...
                    ${
                      hasDayView
                        ? `
                            <div class="journey-map-floating-group">
                              <span class="journey-map-floating-label">显示方式</span>
                              <div class="journey-map-floating-row journey-live-map-day-modes">
                                <button class="journey-map-day-mode-btn active" type="button" data-map-day-mode="solo">单独显示</button>
                                <button class="journey-map-day-mode-btn" type="button" data-map-day-mode="fade">突出显示</button>
                              </div>
                            </div>
                    ...
                    <div class="journey-map-floating-group">
                      <span class="journey-map-floating-label">地图操作</span>
                      <div class="journey-map-floating-row">
                        <button class="journey-map-action-btn active" type="button" data-map-action="route">
                          主路线
                        </button>
                    ...
                    <div class="journey-map-floating-group">
                      <span class="journey-map-floating-label">底图样式</span>
                      <div class="journey-map-floating-row journey-live-map-styles">
                        <button class="journey-map-style-btn active" type="button" data-map-style="standard">
                          标准
                        </button>
                        <button class="journey-map-style-btn" type="button" data-map-style="terrain">
                  ...
                  <div class="journey-map-stage-rail">
                    <div class="journey-map-stage-rail-head">
                      <span>路线顺序</span>
                      <strong class="journey-map-stage-rail-title">总览路线顺序</strong>
                    </div>
                    <div class="journey-map-stage-rail-list">
                      ${routeStops
                        .filter((item) => item.value)
                        .map(
                          (stop, index) => `
                  ...
                  <div class="journey-live-map-footer">
                    ...
                    <div class="journey-map-focus-rail">
                      <button class="journey-map-focus-btn" type="button" data-map-focus="origin">定位出发</button>
                      <button class="journey-map-focus-btn" type="button" data-map-focus="destination">定位目的地</button>
                      <button class="journey-map-focus-btn" type="button" data-map-focus="stay">定位落脚点</button>
      ...
      function renderJourneyPreview(previewState) {
        ...
        const inferredOrigin =
          dayPlans?.[0]?.waypoints?.[0] || cityPair?.origin || "";
        const inferredDestination =
          dayPlans?.[dayPlans.length - 1]?.waypoints?.slice(-1)?.[0] ||
          cityPair?.destination ||
          destinationSection?.title ||
          "";
        const normalizedOrigin = cleanJourneyLocationValue(inferredOrigin || "");
        const normalizedDestination = cleanJourneyLocationValue(
          inferredDestination || ""
        );
        const previewMetrics = [
          {
            ...
            label: "主路线",
            value:
              !isJourneyPlaceholderValue(normalizedOrigin) &&
              !isJourneyPlaceholderValue(normalizedDestination)
          ...
          },
          {
            ...
            label: "优先交通",
            value: transportSection?.title || "待继续比交通方式",
          },
          {
            icon: "fa-bed",
            label: "落脚节奏",
        ...
        const previewStops = [
          {
            ...
            value: cleanJourneyLocationValue(normalizedOrigin || "待确认出发地"),
            note: !isJourneyPlaceholderValue(normalizedOrigin)
              ? "先从这里出发，后面我会继续补齐更细的时间与方式。"
              : "如果你告诉我从哪里走，我会把整段路线串得更完整。",
          },
          {
            label: "目的地",
            icon: "fa-map-pin",
            value: cleanJourneyLocationValue(
              normalizedDestination || "待确认目的地"
            ),
            note: truncateJourneyNote(
          ...
          <section class="journey-preview-board">
            <div class="journey-preview-head">
              <div class="journey-preview-title">
                <strong>路线预览</strong>
                <span>这一块现在先作为路线工作区来用，先定主线、再切分日程、最后继续细化成完整旅游规划。</span>
              </div>
              <div class="journey-preview-badge">
                <i class="fa-solid fa-map-location-dot"></i> 地图路线工作区
              </div>
            </div>
            ${atlasHtml}
          </section>
