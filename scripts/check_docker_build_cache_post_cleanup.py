"""Validate post-cleanup Docker build-cache evidence.
...
DOCKER_BUILD_CACHE_POST_CLEANUP_VERSION = "docker_build_cache_post_cleanup.v1"
...
def _execution_summary(report: Mapping[str, Any] | None) -> dict[str, Any]:
    if report is None:
        return {
            "status": "blocked",
            "blocked_reasons": [_blocker("execution", "missing_execution", "Docker build-cache cleanup execution report is missing.")],
        }
    policy = _as_mapping(report.get("policy"))
    prune = _as_mapping(report.get("prune"))
    build_cache = _as_mapping(report.get("build_cache"))
    blocked: list[dict[str, str]] = []
    degraded: list[dict[str, str]] = []
    ...
    if _status(prune.get("result")) != "passed":
        blocked.append(_blocker("execution", "builder_prune_not_passed", "Docker builder prune did not pass."))
    if _as_mapping(report.get("approval")).get("approval_token_accepted") is not True:
        blocked.append(_blocker("execution", "approval_token_not_accepted", "Execution report must show approval token accepted."))
    if policy.get("deletes_build_cache") is not True:
        blocked.append(_blocker("execution", "build_cache_not_deleted", "Execution report must delete build cache."))
    if policy.get("runs_system_prune") is not False:
        blocked.append(_blocker("execution", "system_prune_used", "Execution report must not run docker system prune."))
    if policy.get("deletes_images") is not False or policy.get("deletes_containers") is not False or policy.get("deletes_volumes") is not False:
        blocked.append(_blocker("execution", "unexpected_resource_delete", "Execution must not delete images, containers or volumes."))
    after = _as_mapping(build_cache.get("after"))
    reclaimable_after = _to_float(after.get("reclaimable_mb"))
    if reclaimable_after > 0:
        degraded.append(_blocker("execution", "build_cache_still_reclaimable", "Docker build cache remains reclaimable after cleanup."))
    return {
        "status": "blocked" if blocked else ("degraded" if degraded else "passed"),
        "mode": _status(report.get("mode")),
        "prune_result": _status(prune.get("result")),
        "approval_token_accepted": _as_mapping(report.get("approval")).get("approval_token_accepted") is True,
        "before_reclaimable_mb": _to_float(_as_mapping(build_cache.get("before")).get("reclaimable_mb")),
        "after_reclaimable_mb": reclaimable_after,
        "estimated_reclaimable_delta_mb": _to_float(build_cache.get("estimated_reclaimable_delta_mb")),
        "blocked_reasons": blocked,
        "degraded_reasons": degraded,
    }
...
def build_docker_build_cache_post_cleanup_report(
    *,
    ...
    if restore.get("status") == "blocked" or capacity.get("status") in {"blocked", "degraded"}:
        ...
        decision = "build_cache_cleanup_incomplete_manual_review_required"
    elif status == "passed":
    ...
    return {
        ...
        "degraded_reasons": degraded,
        "next_actions": [
            "Rerun server capacity snapshot after build-cache cleanup.",
            "Rerun restore drill feasibility with the post-cleanup capacity report.",
            "Refresh M1 go/no-go evidence before widening traffic.",
            "If capacity remains degraded, request storage expansion or Docker data-root migration.",
        ],
        "not_proven_by_this_report": [
            "This report does not delete build cache or free disk space.",
...
def build_docker_build_cache_post_cleanup_markdown(report: Mapping[str, Any]) -> str:
    ...
    lines = [
        "# Docker Build Cache Post-Cleanup Check",
        "",
        ...
        "|---|---|---|",
        f"| Execution | `{_cell(execution.get('status'))}` | prune={_cell(execution.get('prune_result'))}, reclaimed={_cell(execution.get('estimated_reclaimable_delta_mb'))} MB |",
...
def main(argv: Iterable[str] | None = None) -> int:
    ...
    report = build_docker_build_cache_post_cleanup_report(
    ...
    output_text = (
        build_docker_build_cache_post_cleanup_markdown(report)
        if args.markdown
