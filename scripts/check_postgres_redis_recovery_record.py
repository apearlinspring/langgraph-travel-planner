...
def _read_json(path: Path) -> dict[str, Any]:
    try:
        ...
        raise ValueError(f"Cannot read PostgreSQL/Redis recovery record JSON: {path}") from exc
    if not isinstance(payload, dict):
        raise ValueError("PostgreSQL/Redis recovery record must be a JSON object.")
    return payload
