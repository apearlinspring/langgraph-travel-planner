...
def _read_json(path: Path) -> dict[str, Any]:
    try:
        ...
        raise ValueError(f"Cannot read concurrency/rate-limit evidence record JSON: {path}") from exc
    if not isinstance(payload, dict):
        raise ValueError("Concurrency/rate-limit evidence record must be a JSON object.")
    return payload
