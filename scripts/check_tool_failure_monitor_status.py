...
FAILURE_STATUSES = {"failed", "timeout", "degraded", "skipped", "approval_required"}
REQUIRED_DB_ENV = (
    "POSTGRES_HOST",
    "POSTGRES_PORT",
    "POSTGRES_DB",
...
def _summarize_tool_audit_rows(
    ...
    *,
    ...
    failures = [
        row
        for row in rows
        if str(row.get("status") or "").strip().lower() in FAILURE_STATUSES
    ]
    latencies = [
    ...
    failure_error_counts: Counter[str] = Counter(
        _redact_text(str(row.get("error_type") or "unknown_error").strip() or "unknown_error")
        for row in failures
    )
    return {
        "sample_count": total,
        ...
        blockers.append(
            {
                ...
                "finding": "Tool failure rate exceeded the blocking threshold.",
            }
        )
        ...
        degraded.append(
            {
                ...
                "finding": "Tool failure rate exceeded the warning threshold.",
            }
        )
...
def parse_args(argv: Iterable[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--lookback-hours", type=float, default=24, help="Tool audit lookback window.")
    ...
    parser.add_argument("--warn-failure-rate", type=float, default=0.2, help="Warning failure-rate threshold.")
    parser.add_argument("--max-failure-rate", type=float, default=0.5, help="Blocking failure-rate threshold.")
